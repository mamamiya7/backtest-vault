...
3. **Set up everything on one page:** **Test setup** contains universe and the complete date range. **Portfolio** contains capital, allocation, ranking and position limits. **Strategy details** contains periods, weights, indicators, entry chart and timeframe, with Radar, Relative Strength and its System Builder together. Open the separate **Market Trend Filter** window for its independent chart and conditional settings. **Entry conditions** shows Strategy 1–3; **Exit conditions** contains the exit chart, rule, target and stop loss. Each eligible field retains Test values, and checkboxes have separate On/Off test alternatives. Portfolio defaults are checked against RZone before submission. Search and select the universe with the mouse or Arrow keys and Enter; execution still resolves the exact source autocomplete choice.
...
Dropdown choices are cached for the **local calendar day** across page refreshes and RZone tabs. The initial connection checks Candle, P&F and Renko in bounded steps and restores original source settings. Group, Radar My, strategy categories, keyword-search results and loaded Relative Strength/Market Trend Filter choices are retained locally. Reconnecting reads current fields while reusing the matching menus. Already-checked chart coverage prevents another automatic scan of all three charts. A new structural filter context or search needs its first lookup. A cache never substitutes for exact source read-back and symbol identity checks before submission.

Use **Recheck all choices** to discard the daily menus and rescan, including after switching RZone accounts or editing source rules. A new day or adapter update also requires a fresh check. RZone does not expose a stable account identifier on this page; the cache belongs to this browser profile. Changed controls stop reuse with a request to recheck rather than silently walking every menu again. A failed chart scan leaves your current editable form available and names the incomplete chart. Keep source controls unchanged while discovery works. Search-based My/Public categories still require a keyword; the daily check does not invent a complete list for them.

The editor exposes P&F box/reversal controls, Renko brick size/mode, Running/Fresh and the observed numeric strategy inputs. Main chart and execution chart remain independent; switching one does not silently switch the other. Chart and Renko mode remain fixed within a batch, with separate drafts for each context. **Version 0.19.0 adds automated P&F/Renko execution. Three-trial batches for each family passed installed live verification; broader combinations are covered by simulated checks.**
...
Group discovery opens only its own search menu, clears the query to read the exposed full list, restores the original text and closes that menu before reading execution settings. It verifies that the main settings stayed unchanged and rejects ambiguous, missing or oversized lists. Existing reports/menus are never dismissed to make connection succeed. A refreshed source session requires reconnection, but a valid daily Group catalogue is reused. Older saved setups without a Group catalogue retain their original validation behavior.
...
| --- | --- | --- |
| Main chart and Group | Candle / P&F / Renko × four markets | NSE editor and Price execution for all three charts |
| STR1–3 | Three rows × four rule sources × three charts | Three chart-specific editors, My/Public search and Price execution |
| Radar | Pre / My under all three charts | NSE editor for all three charts; an empty native menu stays empty |
| Relative Strength | Separate chart rule families, five benchmark markets, exact benchmark selection | Full editor, Test values and NSE/Price runner |
| Market Trend Filter | Three charts × Index/RS × four actions; four methods; exit categories; both Renko construction blocks | Full editor, Test values and complete capture/replay with the NSE/Price runner |
| Backtest / exits | Three execution charts × Price/RS/Both × four rule sources; cross-check under all three main charts | All three Price editors and execution adapters, including exit My/Public search |
...
| P&F and Renko | Chart-specific editors, independent main/exit charts and automatic execution | Three-trial batches verified live; preserve construction modes and exact submitted settings |

The new-test adapter supports independent **Candle, P&F and Renko charts with Price selection**. Version 0.19.0 passed installed three-trial P&F and Renko batch verification. Other selection modes remain unavailable where their dependent execution needs a separate adapter. Market, chart/selection, Renko construction modes and source-menu parents stay fixed during a batch. Universe and Timeframe support every selected combination. Dates and portfolio assumptions may vary; results with different comparison conditions remain in separate groups. Choose a strategy category from its loaded choices, then select the rules to test. In the saved-run route, inactive fields remain excluded from that baseline's sweep picker. Named rules are recorded as names; hidden rule definitions are never inferred.
...
Live acceptance on 2026-09-16 separately verified three sequential Candle period trials through the **v0.7.3 saved-baseline flow** in the installed extension. The user initiated the batch and exported its records; source operation and capture proceeded automatically. Export verification covered the approved values and fixed settings, unique strategy/portfolio submission IDs, ordered source lifecycle timestamps, complete trade counts, six chart snapshots per run and acknowledged saves before the next trial. Source metrics matched live observations and two independent manual reference calculations. The installed dashboard itself was not directly inspected by automation. This earlier result is separate from the full-setup checks below.

Version 0.19.0 additionally passed three P&F and three Renko trials from **New test**, plus a mixed P&F-entry/Renko-exit run. The P&F batch varied main box sizes 0.25/0.5/1; Renko varied Percent main brick sizes 0.5/1/2. Each exported record matched the approved settings and fresh submission evidence, contained every reported trade and six charts, and was durably saved before the next claim. The Renko and mixed runs included Radar and a Popular exit rule. These batches used NSE/Price with RS and Market Trend Filter off. Additional filter states and Renko modes passed isolated automated fixtures; they are not exhaustive live-service acceptance. Private receipts and reports are not published.

## Remaining delivery plan

1. **Broaden Vault-first live coverage:** the P&F/Renko full-setup batches above pass. Extend installed checks to additional chart/filter contexts and recovery paths; retain the earlier Candle saved-baseline result as separate evidence.
2. **Broaden variant construction coverage:** live Percent Renko and P&F box sweeps pass. Add live Absolute/ATR/ATR % and additional filter/price-mode scenarios when needed; their current coverage is automated fixtures.
