...
Open `?demo=1&view=experiments` and choose **Start a new test**. The single form follows RZone's arrangement. Beside Period 1, open **Test values** and enter `252,500`, then choose **Done**. Click **Backtest**, open the stop-loss Test values, choose **Range**, and enter From `8`, To `10`, Step `2`. The count becomes four tests. Choose **Generate 4 sample results**; the completed ranking displays each trial's exact settings. The leading sample is only an illustration.

**Use a saved run** remains available for the earlier baseline workflow. **Sample results ready** always means fictional examples were generated, not that Definedge executed a backtest. Stop sample generation and resume retain finished samples in memory. A later validation candidate is a separate stage. These series illustrate the interface and do not execute strategy logic or contact RZone. Reset demo clears them.
