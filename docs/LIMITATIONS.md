...
- Daily menus are scoped to the same RZone document session. Reloading RZone or changing its adapter/context can require another scan that day. My/Public rule searches still require a keyword; RZone does not expose all possible search results without one. Recheck all choices forces a full native-menu refresh.
