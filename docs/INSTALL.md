...
The editor supports NSE and Price selection under Candle, P&F and Renko, with separate main/execution chart controls. The first connection checks all three chart menus. Dropdown choices, Group, private categories and loaded filter symbols are reused for the local day across RZone refreshes; current settings are read freshly. Recheck all choices forces a fresh scan, including after switching source accounts or changing rules. Version 0.19.0 enables P&F/Renko automatic execution; three-trial batches for both chart families passed installed live verification. Refresh RZone after activating the update so it advertises the new runner capabilities. No Recheck is needed solely for this update.
...
Reload the existing Backtest Vault extension on Chrome's Extensions page, refresh RZone, and reopen Vault. Preserve any pending recovery download before refreshing. Choose **New test** for the guided setup; **My studies → Use a saved run** keeps the existing-baseline route. Live controls are available only inside the installed extension. Begin with a small Candle/Price test. P&F and Renko are also available with independent main/exit settings; chart construction mode stays fixed within a batch. See [Studies, tests and results](EXPERIMENTS.md).
