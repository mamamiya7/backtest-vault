...
3. Set up the strategy in the RZone-style form. Search and select **Group** from its dropdown. Choose **Off** to skip a period/filter, **On** to include it, or **Test both** to compare separate On and Off runs. Add **Test values** beside a number or menu to compare values or a range. Use **Refresh choices** to reread your account's current menus.
