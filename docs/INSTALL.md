...
Download the complete **backtest-vault-0.11.0.zip** asset from [Releases](https://github.com/mamamiya7/backtest-vault/releases/latest), extract everything, and open the **BacktestVault** folder. Do not download only a setup script. Windows and Linux use the same ZIP.
...
| Dependent choices never finish loading in an older version | Update to v0.11.0, reload the existing extension and refresh RZone/Vault. My and Public use searchable fields, which older readers incorrectly expected to be dropdowns. |
