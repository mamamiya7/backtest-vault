...
3. Set up the strategy in the RZone-style form. Search and select **Group**. For **Radar** or **Strategy 1, 2 or 3**, choose **On**, then a category and rule. Native dropdowns load automatically, even if source rows started off. For STR **My / Public**, enter a name or keyword and click **Search**, then select a matching rule. Choose **Off** to skip a period/filter or **Test both** to compare separate On and Off runs. Add **Test values** beside a number or menu to compare values or a range. Use **Refresh choices** after adding or changing rules in RZone.
...
| A dropdown is missing a recently added rule | Use **Refresh choices**, then choose the intended category. For STR **My / Public**, enter a keyword and click **Search**. No matches refers to that query; unavailable old selections need review. |
| Dependent choices never finish loading in an older version | Update to v0.8.8, reload the existing extension and refresh RZone/Vault. My and Public use searchable fields, which older readers incorrectly expected to be dropdowns. |
