...
| A dropdown is missing a recently added rule | Use **Refresh choices**, then choose the intended category. For strategy/exit **My / Public**, enter a keyword and click **Search**. No matches refers to that query; unavailable old selections need review. |
| Dependent choices never finish loading in an older version | Update to v0.9.0, reload the existing extension and refresh RZone/Vault. My and Public use searchable fields, which older readers incorrectly expected to be dropdowns. |
