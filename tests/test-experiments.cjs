...
(async()=>{setupVariationTests();setupCachedCategoryTests();setupContextVariationTests();await coordinatorTests();await decisionTests();await setupBridgeTests();await ruleSearchBridgeTests();console.log('PASS: bounded numeric/boolean/source-enum/date variations, all-combination dates, matched-scope decisions, forward-window isolation, cached strategy-category isolation, native search types and authorized lookup bridge, immutable settings, queue ownership/recovery, saved evidence and Vault-first setup.');})().catch(e=>{console.error(e);process.exitCode=1;});
