...
// Mounted RS headings can survive switching the filter off; only those observed
// decorations may differ during read-back, never settings, rule identity or labels elsewhere.
{
 const S=require('../dist/setup.js'),L=require('../dist/source-layouts.js'),F=require('./fixtures/filter-setup.cjs');
 for(const chart of ['P&F','Renko']){
  const source=F({chart}),expected=clone(source.stages.momentum.fields),actual=clone(expected),layout=L.stage('momentum',expected);
  actual[layout.rsIndex].label+=' / NSE';
  for(const index of [layout.sizeIndex,layout.modeIndex])actual[index].label='RS SB : / Publici → '+actual[index].label;
  const before={expected:clone(expected),actual:clone(actual)};assert.equal(E.verify(expected,actual),true,'A prior RS Public visit does not invalidate a matching RS-off trial');
  assert.deepEqual({expected,actual},before,'Read-back comparison preserves original archive labels');
  for(const mutate of [
   f=>{f[layout.sizeIndex].label='RS SB : / Publici → Unrelated size :';},
   f=>{f[layout.sizeIndex].value=String(Number(f[layout.sizeIndex].value)+1);},
   f=>{f[layout.modeIndex].value=chart==='P&F'?'9':'ATR';},
   f=>{f[12].label='RS SB : / Publici → '+f[12].label;},
   f=>{f[layout.priceIndices[0]].label='Different price control';},
   f=>{f[layout.rsIndex].label='Relative Strength : / UNKNOWN';}
  ]){const wrong=clone(actual);mutate(wrong);assert.throws(()=>E.verify(expected,wrong),/layout|read-back/,'Known decorations never hide setting or semantic label drift');}
  const on=clone(source.stages.momentum.relativeStrength.fields),wrongCategory=clone(on);wrongCategory[60].value='Popular';assert.throws(()=>E.verify(on,wrongCategory),/read-back/,'RS category values remain exact');
  const wrongRule=clone(on);wrongRule[61].value='Different native rule';assert.throws(()=>E.verify(on,wrongRule),/read-back/,'RS rule identity remains exact');
 }
}
...
async function refreshedSourceRecoveryTests(){
 const S=require('../dist/setup.js');
 async function fixture(){
  const memory={},runtime={id:'refresh-extension',getURL:p=>'chrome-extension://refresh-extension/'+p},dashboard={id:runtime.id,url:runtime.getURL('index.html')},source={id:runtime.id,url:'https://zone.definedgesecurities.com/index.html#research',tab:{id:7}};
  let time=100000,session='original-document',rejectProbe=false,rejectWrite=false,configured=0,ids=0,ready=true;
  const storage={get:async key=>clone(key?{[key]:memory[key]??null}:memory),set:async values=>{if(rejectWrite)throw Error('Storage unavailable');Object.assign(memory,clone(values));}};
  const make=()=>createCoordinator({storage,runtime,clock:()=>time,uuid:()=> 'refresh-'+(++ids),probe:async()=>{if(rejectProbe)throw Error('No reply');return {session,ready,capable:ready,chart:'Candle',reason:ready?'':'Open Momentum BackTesting in RZone.'};},configure:async()=>{configured++;const config=S.demoTemplate();config.demo=false;config.session=session;return {ok:true,session,config};}});
  let c=make();const call=(action,data={},sender=dashboard)=>c.handle({action,...data},sender),real=clone(b);real.demo=false;
  const plan=(await call('create',{plan:{...config,baseline:real}})).experiment;
  await call('hello',{session,ready:true,capable:true,chart:'Candle'},source);await call('start',{id:plan.id,tabId:7});
  return {memory,call,plan,source,configured:()=>configured,restart:()=>{c=make();},setSession:value=>{session=value;},setReady:value=>{ready=value;},expire:()=>{time+=90001;},rejectProbe:value=>{rejectProbe=value;},rejectWrite:value=>{rejectWrite=value;}};
 }
 for(const stage of E.active){
  const f=await fixture(),claim=await f.call('claim',{id:f.plan.id,session:'original-document'},f.source),args={id:f.plan.id,session:'original-document',trialId:claim.trial.id,token:claim.token};
  for(const next of E.active.slice(1,E.active.indexOf(stage)+1))await f.call('checkpoint',{...args,stage:next},f.source);
  const before=clone(f.memory['experiment:'+f.plan.id]),cache={day:'today',sentinel:'Keep dropdown choices'},saved={id:'earlier-result',note:'Keep all saved evidence'};f.memory['runner:choices:daily']=cache;f.memory['run:earlier-result']=saved;
  // v0.18.2 already overwrote this receipt but left old study ownership behind.
  f.memory['runner:tab:7'].session='refreshed-document';f.setSession('refreshed-document');const hello=await f.call('hello',{session:'refreshed-document',ready:true,capable:true},f.source);
  assert.equal(hello.id,null,'A refreshed source must not automatically resume an interrupted study');
  const recovered=f.memory['experiment:'+f.plan.id];assert.equal(recovered.status,'needs-review');assert.equal(recovered.owner,undefined);assert.equal(f.memory['runner:lease'],null,'Confirmed replacement releases only the dead document lease');
  assert.equal(recovered.trials[0].status,'uncertain');assert.match(recovered.trials[0].error,/refreshed|reopened/);assert.deepEqual(recovered.trials[0].execution,before.trials[0].execution);assert.deepEqual(recovered.trials.slice(1),before.trials.slice(1),'Queued trials are never claimed or replayed during reconnect');assert.deepEqual(recovered.baseline,before.baseline);assert.deepEqual(f.memory['run:earlier-result'],saved);assert.deepEqual(f.memory['runner:choices:daily'],cache);
  assert.equal((await f.call('configure',{tabId:7})).source.session,'refreshed-document','New test can connect after refresh at every submission stage');
  await assert.rejects(f.call('checkpoint',{...args,stage:'strategy-submitting'},f.source),/does not own/,'Old callbacks cannot reclaim the released source');await assert.rejects(f.call('start',{id:f.plan.id,tabId:7}),/Review interrupted/);
 }
 // A queued study can also survive a refresh before its first claim.
 const waiting=await fixture();waiting.setSession('new-document');await waiting.call('hello',{session:'new-document',ready:true},waiting.source);assert.equal(waiting.memory['experiment:'+waiting.plan.id].status,'paused');assert.equal(waiting.memory['experiment:'+waiting.plan.id].owner,undefined);assert.ok(waiting.memory['experiment:'+waiting.plan.id].trials.every(t=>t.status==='queued'));const idleReceipt=clone(waiting.memory);await waiting.call('hello',{session:'original-document',ready:true},waiting.source);assert.deepEqual(waiting.memory,idleReceipt,'Late hello cannot hide the refreshed tab even between studies with no lease');await waiting.call('configure',{tabId:7});
 // A fully verified saved report is recovered without recomputing it.
 const durable=await fixture(),claimed=await durable.call('claim',{id:durable.plan.id,session:'original-document'},durable.source),owned={id:durable.plan.id,session:'original-document',trialId:claimed.trial.id,token:claimed.token};
 for(const stage of E.active.slice(1))await durable.call('checkpoint',{...owned,stage},durable.source);
 const run=recordedResult(durable.plan,claimed.trial);durable.memory['run:'+run.id]=clone(run);await durable.call('pause',{id:durable.plan.id});durable.setSession('new-document');await durable.call('hello',{session:'new-document',ready:true},durable.source);assert.equal(durable.memory['experiment:'+durable.plan.id].trials[0].status,'saved');assert.equal(durable.memory['experiment:'+durable.plan.id].status,'paused');assert.deepEqual(durable.memory['run:'+run.id],run);
 const unproven=await fixture(),attempt=await unproven.call('claim',{id:unproven.plan.id,session:'original-document'},unproven.source),incomplete=recordedResult(unproven.plan,attempt.trial);delete incomplete.experiment.evidence;unproven.memory['run:'+incomplete.id]=clone(incomplete);unproven.setSession('new-document');await unproven.call('hello',{session:'new-document',ready:true},unproven.source);assert.equal(unproven.memory['experiment:'+unproven.plan.id].trials[0].status,'uncertain','An unverified saved record cannot be promoted during recovery');assert.deepEqual(unproven.memory['run:'+incomplete.id],incomplete);
 // Service worker restart and another source tab are not evidence of a dead owner.
 const live=await fixture(),active=await live.call('claim',{id:live.plan.id,session:'original-document'},live.source);live.restart();const lease=clone(live.memory['runner:lease']);await live.call('hello',{session:'original-document',ready:true},live.source);await live.call('hello',{session:'another-tab',ready:true},{...live.source,tab:{id:8}});await assert.rejects(live.call('configure',{tabId:7}),/Finish or pause/);assert.equal(live.memory['experiment:'+live.plan.id].trials[0].status,'applying');assert.deepEqual(live.memory['runner:lease'],lease);
 live.rejectProbe(true);live.expire();await assert.rejects(live.call('configure',{tabId:7}),/review|Finish or pause/i);assert.deepEqual(live.memory['runner:lease'],lease,'Timeout or failed probe alone never releases an uncertain submission');assert.equal(live.configured(),0);
 // A direct, authenticated status reply can recover before the next heartbeat.
 const early=await fixture();await early.call('claim',{id:early.plan.id,session:'original-document'},early.source);early.setSession('new-document');await early.call('list');assert.equal(early.memory['runner:lease'],null);assert.equal(early.memory['experiment:'+early.plan.id].status,'needs-review');
 const direct=await fixture();await direct.call('claim',{id:direct.plan.id,session:'original-document'},direct.source);direct.setSession('new-document');assert.equal((await direct.call('configure',{tabId:7})).source.session,'new-document','Connect itself recovers before any list or new hello');
 const landing=await fixture();landing.setSession('new-document');landing.setReady(false);await assert.rejects(landing.call('configure',{tabId:7}),/Open Momentum BackTesting/);assert.equal(landing.memory['experiment:'+landing.plan.id].status,'paused');assert.equal(landing.configured(),0,'A new source still on its landing page cannot be configured');
 // Late messages from the old document cannot stop a newly started study.
 const delayed=await fixture();delayed.setSession('new-document');await delayed.call('hello',{session:'new-document',ready:true},delayed.source);await delayed.call('start',{id:delayed.plan.id,tabId:7});await delayed.call('claim',{id:delayed.plan.id,session:'new-document'},delayed.source);const liveState=clone(delayed.memory);assert.equal((await delayed.call('hello',{session:'original-document',ready:true},delayed.source)).id,null);assert.deepEqual(delayed.memory,liveState);
 await delayed.call('hello',{session:'new-document',ready:true},delayed.source);assert.deepEqual(delayed.memory,liveState,'Repeated current-session heartbeats do not re-journal or replay recovery');
 const conflict=await fixture();await conflict.call('claim',{id:conflict.plan.id,session:'original-document'},conflict.source);conflict.memory['experiment:'+conflict.plan.id].owner={tabId:8,session:'different-owner'};conflict.setSession('new-document');const conflictBefore=clone(conflict.memory);await assert.rejects(conflict.call('configure',{tabId:7}),/Finish or pause/);assert.deepEqual(conflict.memory,conflictBefore,'Conflicting live owner metadata stays blocked for review');
 // Saving the interrupted journal and releasing its lease are one write.
 const failure=await fixture();await failure.call('claim',{id:failure.plan.id,session:'original-document'},failure.source);const beforeFailure=clone(failure.memory);failure.rejectWrite(true);failure.setSession('new-document');await assert.rejects(failure.call('hello',{session:'new-document',ready:true},failure.source),/Storage unavailable/);assert.deepEqual(failure.memory,beforeFailure,'A failed recovery write must not partially free ownership');
 console.log('PASS: refreshed source recovery at every trial stage, preserved evidence/cache, durable reconciliation, no replay and live-owner protection.');
}
