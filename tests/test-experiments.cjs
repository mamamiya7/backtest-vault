...
async function symbolSearchBridgeTests(){
 ...
 const first=await search();assert.equal(first.result.options[0].sourceValue,'native:101');assert.deepEqual(calls.at(-1).request,{kind:'symbol',stage:'momentum',fieldIndex:rs.benchmarkIndex,market:'NSE',query:'Fictional',session});assert.equal((await search({market:'All'})).result.options[0].market,'NSE','All-market results retain their exact actual exchange');
 for(const fieldIndex of [mtf.indexSymbolIndex,mtf.numeratorSymbolIndex,mtf.denominatorSymbolIndex])assert.equal((await search({stage:'marketFilter',fieldIndex})).result.fieldIndex,fieldIndex);
 ...
 replySession='another-document';await assert.rejects(search(),/reloaded during/);replySession=session;
 for(const patch of [{stage:'execution'},{fieldIndex:1},{market:'BSE'},{query:'different'},{controlType:'select-one'},{options:{}},{options:Array(3001).fill({})}]){responsePatch=patch;await assert.rejects(search(),/different symbol search/);}responsePatch=null;
 for(const patch of [{sourceValue:undefined},{sourceValue:''},{value:'Different label'},{market:'BSE'},{label:'bad\nlabel'},{disabled:'false'}]){resultOptions=[{value:'Fictional index',label:'Fictional index',sourceValue:'native:101',market:'NSE',disabled:false,...patch}];await assert.rejects(search(),/different symbol search/);}
 resultOptions=[{value:'Fictional foreign',label:'Fictional foreign',sourceValue:'native:102',market:'unoffered'}];await assert.rejects(search({market:'All'}),/different symbol search/);resultOptions=[];assert.deepEqual((await search()).result.options,[],'A confirmed empty query response is an empty list, not a fabricated available symbol');resultOptions=undefined;
