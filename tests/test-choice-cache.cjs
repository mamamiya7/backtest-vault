...
(async()=>{
 ...
 assert.deepEqual(Object.keys(h.memory).sort(),[key,sharedKey,'runner:lookup:'+tabId,'runner:tab:'+tabId].sort(),'Choice reads store lookup routes but cannot create runs, experiments or execution leases');
 assert.deepEqual(Object.keys(h.memory['runner:lookup:'+tabId]).sort(),['rules','session','symbols'],'Lookup authorization retains only control routes, not source settings or query results');
