...
async function filterScenario(mode){
 ...
 const nativeTimeout=w.setTimeout.bind(w),nativeInterval=w.setInterval.bind(w);w.setTimeout=(fn,ms)=>nativeTimeout(fn,Math.min(ms,20));w.setInterval=(fn,ms)=>nativeInterval(fn,Math.min(ms,30));
 let groupCommits=0,orderClicks=0,submissions=0,portfolios=0,filterSaves=0,symbolQueryClears=0,changeSymbolIdentity=false,changeSymbolExchange=false,mainState=E.clone(sourceTemplate.stages.momentum.fields),rsState=E.clone(sourceTemplate.stages.momentum.relativeStrength.fields),filterState=E.clone(sourceTemplate.stages.marketFilter.current.fields),filterFull=E.clone(sourceTemplate.stages.marketFilter.fields);
 ...
 const selectionIDs=()=>JSON.stringify([...symbolSelections].sort(([a],[b])=>a.localeCompare(b))),originalSelectionIDs=selectionIDs(),noSymbolMenus=()=>assert.equal([...d.querySelectorAll('[data-symbol-menu]')].filter(menu=>menu.isConnected).length,0,'No symbol popup may remain after its source operation or parent dialog closes.');
 ...
 function render(p,stage,snapshot){
  ...
  for(const index of symbolIndices){
   ...
   const query=()=>{
    ...
    if(!n.value){const retained=symbolSelections.get(owner);for(const menu of existing)menu.remove();if(existing.length)symbolQueryClears++;assert.equal(symbolSelections.get(owner),retained,'Clearing a transient query does not choose or change a source symbol.');return;}
    const menu=existing[0]||d.createElement('div');menu.replaceChildren();menu.className='popupContent';menu.dataset.symbolMenu=owner;const list=d.createElement('ul');list.className='ind-list';menu.append(list);menus.add(menu);
    const exchange=changeSymbolExchange?'BSE':current[index-1].selectedOptions[0].textContent==='All'?'NSE':current[index-1].selectedOptions[0].textContent,label=n.value,id=label+'|'+(changeSymbolIdentity?'changed:':'')+owner,li=d.createElement('li');li.setAttribute('name',id);li.setAttribute('exhg',exchange);const select=d.createElement('div');select.className='symbol-search-list';const name=d.createElement('div');name.textContent=label;select.append(name);select.onclick=()=>{n.value=label;n.dataset.symbol=id;symbolSelections.set(owner,id);commits.push({stage,index,label,id,exchange});menu.remove();};li.append(select);
    for(const side of ['buy','sell']){const trade=button(li,side,()=>{orderClicks++;throw Error('A trading action must never be used for symbol selection.');});trade.className='btn-'+side;}list.append(li);if(!menu.isConnected)d.body.append(menu);
   };
...
(async()=>{
 ...
 const cases=[
  ...
  ...['relative-strength','market-filter','combined','symbol-identity','market-retained','market-context','market-renko'].map(filterCase=>({filterCase}))
 ];
