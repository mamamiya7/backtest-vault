...
async function filterScenario(mode,dailySeed){
 ...
 function options(stage,layout,index,field){const row=layout?.rows.find(row=>row.parentIndex===index),base=descriptor(stage,layout);if(row)return row.name==='Radar'?['Pre','My']:['Pre','My','Public','Popular'];if(index===layout?.chartIndex)return [field.value];const known=base.options?.[stage==='marketFilter'?canonicalMarketIndex(layout,index):index];return known?.map(o=>typeof o==='string'?o:o.label)||[field.value];}
 ...
 function render(p,stage,snapshot){
  ...
  if(stage==='marketFilter'){
   ...
   if(layout.chart==='Renko')for(const [at,value]of [[layout.modeIndex,layout.sizeIndex],[layout.exitModeIndex,layout.exitSizeIndex]])if(Number.isInteger(at))current[at].addEventListener('change',()=>{current[value].value=['ATR','ATR %'].includes(current[at].selectedOptions[0].textContent)?'14':'1';});
  }
 }
 ...
 try{
  ...
  const finalConfig={...config,'momentum.market-filter':true,...(mode==='combined'?{'momentum.rs':true}:{}),...(mode==='market-retained'?{'marketFilter.method':'D Smart','marketFilter.ema':37,'marketFilter.dsmart':41,'marketFilter.mast':53,'marketFilter.ktqp':67,'marketFilter.index.symbol':'Changed Demo index','marketFilter.numerator.symbol':'Changed Demo numerator','marketFilter.denominator.symbol':'Changed Demo denominator'}:{}),...(mode==='market-renko'?{'marketFilter.brick.size':17,'marketFilter.exit.brick.size':21}:{})},plan=E.create({id:'filter-proof',name:'Verified filter submissions',baseline:S.configToBaseline(finalConfig,normalized,{id:'filter-baseline',name:'Filters',demo:false}),dimensions:[{key:'marketFilter.mode',values:['Index','RS']}],objective:'returns',minTrades:0});memory['experiment:'+plan.id]=plan;
  if(mode==='market-renko'){filterState=L.projectMarketFilter(sourceTemplate.stages.marketFilter.fields,{mode:'Index',action:L.marketActions[2]});const layout=L.marketFilter(filterState);for(const index of [layout.modeIndex,layout.exitModeIndex])filterState[index].value='Percent';for(const index of [layout.sizeIndex,layout.exitSizeIndex])filterState[index].value='1';}
  for(let n=0;n<30&&!memory['runner:tab:9']?.ready;n++)await sleep(10);await coordinator.handle({action:'start',id:plan.id,tabId:9},dashboard);await request({type:'vault-runner-wake'});for(let n=0;n<1600&&!['complete','needs-review'].includes(memory['experiment:'+plan.id].status);n++)await sleep(20);const result=memory['experiment:'+plan.id];assert.equal(result.status,'complete',JSON.stringify(result.trials.map(t=>({status:t.status,error:t.error}))));assert.equal(submissions,2);assert.equal(portfolios,2);assert.equal(filterSaves,2);assert.equal(orderClicks,0);
...
(async()=>{
 ...
 for(const [index,options]of selected.slice(fromCase-1,lastCase).entries()){
  ...
  if(['P&F','Renko','dependent-timeout','daily-passive-state','saved-visited-layout'].includes(options.chartCase)||options.filterCase==='rs-warm')await withFixtureTime(run);else await run();console.log('PASS: runner '+(index+fromCase)+'/'+selected.length+' '+(Object.keys(options).length?JSON.stringify(options):'baseline sequence'));
 }
