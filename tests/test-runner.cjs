...
async function filterScenario(mode){
 ...
 function render(p,stage,snapshot){
  ...
  for(const index of symbolIndices){
   ...
   const query=()=>{
    ...
    if(!n.value){const retained=symbolSelections.get(owner);for(const menu of existing)closeSymbolMenu(menu);if(existing.length)symbolQueryClears++;assert.equal(symbolSelections.get(owner),retained,'Clearing a transient query does not choose or change a source symbol.');return;}
 ...
 try{
  for(let n=0;n<30&&!memory['runner:tab:9'];n++)await sleep(10);const initial=JSON.stringify(fields(main)),loaded=await request({type:'vault-runner-config'});assert.equal(loaded.ok,true,loaded.error);assert.equal(JSON.stringify(fields(main)),initial);assert.equal(submissions,0);let template=loaded.config;
  for(const loadFilter of ['relative-strength','symbol-identity'].includes(mode)?['relativeStrength']:mode.startsWith('market-')?['marketFilter']:['relativeStrength','marketFilter']){const priorClears=symbolQueryClears,response=await request({type:'vault-runner-config',loadFilter});if(['market-unknown-dialog','market-symbol-timeout'].includes(mode)){assert.equal(response.ok,false);if(mode==='market-unknown-dialog'){assert.match(response.error,/unexpected symbol dialog/i);assert.ok(w.VaultCapture.popup('Unexpected source notice'),'Unknown dialogs remain visible for the user rather than being treated as a symbol shell.');}else{assert.match(response.error,/did not return confirmed symbols/i);assert.ok(symbolQueryClears>priorClears,'A spinner that never finishes is still dismissed through its native empty query.');noSymbolMenus();}assert.equal(submissions,0);assert.equal(filterSaves,0);assert.equal(commits.length,0);assert.equal(selectionIDs(),originalSelectionIDs);assert.equal(JSON.stringify(fields(main)),initial);return;}assert.equal(response.ok,true,response.error);template=response.config;assert.equal(JSON.stringify(fields(main)),initial,'Optional discovery restores exact main controls.');assert.equal(w.VaultCapture.popup('Market Trend Filter'),undefined);assert.equal(filterSaves,0,'Discovery closes its owned dialog without committing changed settings.');assert.ok(symbolQueryClears>priorClears,'Native empty-query dismissal is exercised during filter discovery.');assert.equal(selectionIDs(),originalSelectionIDs,'Discovery preserves every original native symbol identity.');noSymbolMenus();}
