...
async function chartScenario(chartCase,dailySeed){
 ...
 const initial=chartCase==='Renko'?'Renko':chartCase==='warm-reversal'?'P&F':'Candle',daily=chartCase==='daily-categories';let executionState=null,privateRadarChoices=chartCase==='cache-reconnect'||daily?['Fictional private radar']:[];const categoryRequests=[],groupRows=['Demo universe 40','Other fictional group'];
...
(async()=>{
 ...
 const cases=[
  ...
  {chartCase:'daily-categories'},{filterCase:'daily-cache'},{vaultSetup:true,closeSlowAnimation:true},{vaultSetup:true,closeBlockedByChild:true},{filterCase:'market-late-symbol'},{filterCase:'rs-warm'},{filterCase:'symbol-no-match'},{chartCase:'daily-storage-order'},{filterCase:'symbol-catalogue'}
 ];
