...
(async()=>{
 ...
 for(const [index,options]of selected.slice(fromCase-1,lastCase).entries()){
  ...
  if(['P&F','Renko','dependent-timeout','daily-passive-state'].includes(options.chartCase)||options.filterCase==='rs-warm')await withFixtureTime(run);else await run();console.log('PASS: runner '+(index+fromCase)+'/'+selected.length+' '+(Object.keys(options).length?JSON.stringify(options):'baseline sequence'));
 }
