...
async function filterScenario(mode){
 ...
 const nativeTimeout=w.setTimeout.bind(w),nativeInterval=w.setInterval.bind(w);w.setTimeout=(fn,ms)=>nativeTimeout(fn,Math.min(ms,20));w.setInterval=(fn,ms)=>nativeInterval(fn,Math.min(ms,30));
 let groupCommits=0,orderClicks=0,submissions=0,portfolios=0,filterSaves=0,changeSymbolIdentity=false,changeSymbolExchange=false,mainState=E.clone(sourceTemplate.stages.momentum.fields),rsState=E.clone(sourceTemplate.stages.momentum.relativeStrength.fields),filterState=E.clone(sourceTemplate.stages.marketFilter.current.fields),filterFull=E.clone(sourceTemplate.stages.marketFilter.fields);
 const nodes=p=>[...p.querySelectorAll('input,select')].filter(n=>n.type!=='hidden'),clearMenus=()=>{for(const menu of menus)menu.remove();},fields=p=>JSON.parse(JSON.stringify(w.VaultCapture.fields(p)));
 ...
 function render(p,stage,snapshot){
  ...
  for(const index of symbolIndices){const n=current[index];n.placeholder='Search Symbol';const query=()=>{clearMenus();if(!n.value)return;const menu=d.createElement('div');menu.className='popupContent';const list=d.createElement('ul');list.className='ind-list';menu.append(list);menus.add(menu);const exchange=changeSymbolExchange?'BSE':current[index-1].selectedOptions[0].textContent==='All'?'NSE':current[index-1].selectedOptions[0].textContent,label=n.value,id=label+'|'+(changeSymbolIdentity?'changed:':'')+stage+':'+['benchmarkIndex','indexSymbolIndex','numeratorSymbolIndex','denominatorSymbolIndex'].find(key=>layout[key]===index),li=d.createElement('li');li.setAttribute('name',id);li.setAttribute('exhg',exchange);const select=d.createElement('div');select.className='symbol-search-list';const name=d.createElement('div');name.textContent=label;select.append(name);select.onclick=()=>{n.value=label;n.dataset.symbol=id;commits.push({stage,index,label,id,exchange});menu.remove();};li.append(select);for(const side of ['buy','sell']){const trade=button(li,side,()=>{orderClicks++;throw Error('A trading action must never be used for symbol selection.');});trade.className='btn-'+side;}list.append(li);d.body.append(menu);};n.addEventListener('keyup',()=>w.setTimeout(query,30));current[index-1].addEventListener('change',()=>{n.value='';delete n.dataset.symbol;});}
  if(stage==='momentum'){
 ...
 try{
  ...
  for(const loadFilter of ['relative-strength','symbol-identity'].includes(mode)?['relativeStrength']:mode.startsWith('market-')?['marketFilter']:['relativeStrength','marketFilter']){const response=await request({type:'vault-runner-config',loadFilter});assert.equal(response.ok,true,response.error);template=response.config;assert.equal(JSON.stringify(fields(main)),initial,'Optional discovery restores exact main controls.');assert.equal(w.VaultCapture.popup('Market Trend Filter'),undefined);assert.equal(filterSaves,0,'Discovery closes its owned dialog without committing changed settings.');}
