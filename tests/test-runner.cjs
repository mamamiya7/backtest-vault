...
async function scenario(options={}){
 const {changeLocked=false,overlap=false,staleCompletion=false,rejected=false,reuseReport=false,noRunning=false,preexistingReport=false,vaultSetup=false,missingGroup=false,changedGroupIdentity=false,changedOptions=false,driftDuringRun=false,refreshParents=false,noOptionRefresh=false,emptyOptions=false,variableSet='',bridge=false,closeAfterWake=false,closeStuckAfterWake=false,closeSlowAnimation=false,closeBlockedByChild=false,groupCatalogue='',ruleCatalogue=''}=options;
 ...
 function popup(title){const p=d.createElement('div');p.className='popupContent';const h=d.createElement('div');h.className='custom-dialog-header';const caption=d.createElement('div');caption.className='caption';caption.textContent=title;const close=d.createElement('a');close.className='close-buton';close.onclick=()=>{
  if(title==='Momentum Trading BackTest')sourceCloseClicks++;
  ...
  if(title==='Momentum Trading BackTest'&&closeSlowAnimation){
   const shell=p.closest('.custom-dialog'),now=w.Date.now.bind(w.Date);shell.style.overflow='hidden';shell.style.clip='rect(0px, 460px, 512px, 0px)';nativeTimeout(()=>{w.Date.now=()=>now()+6000;},5);nativeTimeout(()=>shell.remove(),130);
  }else if(title==='Momentum Trading BackTest'&&(closeAfterWake||closeStuckAfterWake)){
 ...
 };h.append(caption,close);p.append(h);d.body.append(p);if(title==='Momentum Trading BackTest'&&closeSlowAnimation){const shell=d.createElement('div');shell.className='custom-dialog';p.replaceWith(shell);shell.append(p);}return p;}
 ...
 const groupSearches=[],groupRows=['Demo universe 40','Nifty 50 Index','Nifty 500 Index','Other index'],scopeSubmissions=[];let groupMenuReads=0,groupChanges=0,lastGroupQuery,headingClicks=0,openedBeforeInitial=false,groupIdentityChanged=false;
 if(vaultSetup){
  ...
  const render=(names,query)=>{
   ...
   for(const [i,name]of names.entries()){
    const li=d.createElement('li');li.setAttribute('grpid',(groupIdentityChanged?'changed-group:':'group:')+(groupRows.includes(name)?groupRows.indexOf(name):i));li.textContent=name;
    li.onclick=()=>{group.value=name;group.dataset.selectedGroup=li.getAttribute('grpid');groupCommits++;cancelSearches();menu.remove();};list.append(li);
   }
 ...
 cancel=button(main,'BackTest',()=>{settingsReads++;const p=popup('Momentum Trading BackTest');if(closeBlockedByChild)popup('Unexpected source notice');form(p,sourceExecutionFields);button(p,'Backtest',()=>{
  if(variableSet==='universe-timeframe'){const controls=[...main.querySelectorAll('input,select')],group=controls[1].value,timeframe=controls[33].selectedOptions[0].textContent;assert.equal(controls[1].dataset.selectedGroup,'group:'+groupRows.indexOf(group),'Commit the exact observed native Group ID before each submission.');assert.equal(controls[33].value,'source:'+timeframe,'Apply the native timeframe option, not display text as its DOM value.');scopeSubmissions.push({group,timeframe});}
 ...
 if(variableSet==='universe-timeframe'){const table=panel.querySelector('.dropdown-table-body'),scope=scopeSubmissions.at(-1);table.rows[0].cells[1].textContent=scope.group;const row=table.insertRow();row.insertCell().textContent='Timeframe';row.insertCell().textContent=scope.timeframe;}
 ...
 try{for(let n=0;n<50&&!memory['runner:tab:9'];n++)await sleep(10);
  ...
  if(vaultSetup){
   ...
   if(closeBlockedByChild){assert.equal(response.ok,false);assert.match(response.error,/open menu above its settings/);assert.ok(w.VaultCapture.popup('Momentum Trading BackTest'));assert.ok(w.VaultCapture.popup('Unexpected source notice'));assert.equal(sourceCloseClicks,0,'Never click the parent close through an unexpected child popup.');assert.equal(submissions,0);assert.equal(portfolios,0);return;}
   ...
   if(closeAfterWake||closeSlowAnimation){assert.deepEqual(sourceSuccessDialogs,[false]);if(closeSlowAnimation)assert.equal(sourceCloseClicks,1,'A native clipping animation must finish without repeated close clicks.');return;}
   if(bridge){assert.equal(bridgeConfigRequests,3,'Both rejected chart/market changes and the successful connection must cross the real background boundary.');assert.equal(response.config.session,memory['runner:tab:9'].session,'Reading settings must retain the registered document session.');assert.deepEqual(sourceMessages.slice(-4),['Connecting to Vault: opening Momentum settings…','Connecting to Vault: checking current settings…','Connecting to Vault: reading backtest settings…','RZone settings read. Return to Vault to finish setup.']);assert.deepEqual(sourceSuccessDialogs,[false],'Connection success must only appear after closing its own settings dialog.');}
   ...
   if(changedGroupIdentity)groupIdentityChanged=true;
  ...
  for(let n=0;n<(crossRows||exitMode||variableSet==='universe-timeframe'?1200:600)&&!['complete','needs-review'].includes(memory['experiment:'+plan.id].status);n++)await sleep(20);
  ...
  if(['search-ambiguous','exit-search-ambiguous'].includes(ruleCatalogue)){assert.equal(result.status,'needs-review',JSON.stringify(result.trials));assert.match(result.trials[0].error,/more than one RZone choice/);assert.equal(submissions,0);assert.equal(portfolios,0);assert.equal(runs.length,0);assert.equal(ruleSelections.length,0);}
  else if(missingGroup||changedGroupIdentity||changedOptions||driftDuringRun){
   ...
   assert.match(result.trials[0].error,missingGroup?/confirm that group/:changedGroupIdentity?/group.*(?:changed|match|identity)|(?:changed|match|identity).*group/i:changedOptions?/dropdown value is unavailable/:/read-back differs/);if(changedGroupIdentity)assert.equal(groupCommits,0,'An identically labelled but different native Group must not be committed or submitted.');
  }
   ...
   const expectedCount=plan.trials.length;assert.equal(result.status,'complete',JSON.stringify(result.trials.map(t=>({status:t.status,error:t.error}))));assert.equal(submissions,expectedCount);assert.equal(portfolios,expectedCount);assert.equal(runs.length,expectedCount);assert.deepEqual(runs.map(r=>E.fields(r,'momentum')[12].value),variableSet?Array(expectedCount).fill('180'):['126','180','252']);
   ...
   for(const r of runs){assert.equal(r.provenance,'recorded-at-submit');assert.equal(r.charts.length,6);assert.equal(r.trades.rows.length,4);assert.equal(r.trades.rows[1][2],'0');assert.equal(r.experiment.id,plan.id);
    if(vaultSetup){const t=plan.trials.find(t=>t.runId===r.id),expected=E.expected(plan,t);for(const stage of ['momentum','execution','portfolio'])E.verify(E.fields(expected,stage),E.fields(r,stage));assert.equal(E.fields(r,'momentum')[1].value,t.patch['momentum.group']||'Nifty 50 Index');assert.equal(E.fields(r,'execution')[1].value,t.patch['execution.from']||'2023-01-01');assert.equal(E.fields(r,'portfolio')[2].value,String(t.patch['portfolio.capital']||500000));
     if(variableSet==='universe-timeframe'){assert.equal(E.fields(r,'momentum')[33].value,t.patch['momentum.timeframe']);const details=w.Vault.details(r);assert.equal(details.group,t.patch['momentum.group']);assert.equal(details.timeframe,t.patch['momentum.timeframe']);}
   ...
   }assert.equal(new Set(runs.map(r=>r.experiment.evidence.strategySubmissionId)).size,expectedCount);assert.equal(new Set(runs.map(r=>r.experiment.evidence.portfolioSubmissionId)).size,expectedCount);assert.deepEqual(savedBeforeNext,Array(expectedCount-1).fill(true),'Each earlier result is durable before the next source submission.');assert.equal(memory['runner:lease'],null);if(vaultSetup)assert.equal(groupCommits,expectedCount);if(variableSet)for(const dimension of plan.dimensions)assert.equal(new Set(plan.trials.map(t=>t.patch[dimension.key])).size,dimension.values.length,'The trial fixture must exercise every '+dimension.key+' choice.');
   if(variableSet==='universe-timeframe')assert.deepEqual(scopeSubmissions,groupRows.slice(1).flatMap(group=>['Daily','Weekly'].map(timeframe=>({group,timeframe}))),'Execute all three universes × two timeframes in the approved order.');
  }
 }finally{dom.window.close();}
}
// This fixture exercises the production runner/capture/coordinator lifecycle.
// Its reports and source menus are fictional; it does not constitute live RZone acceptance.
async function variantLifecycle({mainChart,exitChart,mainMode='Percent',exitMode='Percent',failure=''}){
 const L=require('../dist/source-layouts.js'),dom=new JSDOM('<body><h1 class="header-text">Momentum Trading BackTesting</h1><div class="account-right"></div></body>',{runScripts:'outside-only',url:'https://zone.definedgesecurities.com/index.html#research'}),w=dom.window,d=w.document,main=d.querySelector('.account-right');
 installFixtureTime(w);w.structuredClone=structuredClone;
 Object.defineProperty(w.HTMLElement.prototype,'innerText',{get(){return this.textContent;}});
 w.Element.prototype.getClientRects=function(){return this.isConnected&&!this.closest('[hidden]')?[{width:100,height:20}]:[];};
 const nodes=p=>[...p.querySelectorAll('input,select')],fields=p=>JSON.parse(JSON.stringify(w.VaultCapture.fields(p))),listeners=[],memory={},submissions=[],portfolioSubmissions=[],durableBeforeNext=[],nativeModes=[];
 let menus=[],oldReport,plan,reportCount=0,groupCommits=0,uid=0;
 const button=(p,label,fn)=>{const b=d.createElement('button');b.textContent=label;b.onclick=fn;p.append(b);return b;};
 function popup(title){const p=d.createElement('div');p.className='popupContent';const head=d.createElement('div');head.className='custom-dialog-header';const caption=d.createElement('div');caption.className='caption';caption.textContent=title;const close=d.createElement('a');close.className='close-buton';close.onclick=()=>p.remove();head.append(caption,close);p.append(head);d.body.append(p);return p;}
 function descriptor(stage,chart,mode){if(chart==='Renko'&&!mode)mode='Percent';return S.demoTemplate({momentumChart:chart,executionChart:chart,...(chart==='Renko'&&mode?{momentumBrickMode:mode,executionBrickMode:mode}:{})}).stages[stage];}
 function render(p,stage,chart='Candle',mode){
  const source=descriptor(stage,chart,mode),layout=stage==='portfolio'?null:L.stage(stage,source.fields),table=d.createElement('table');p.querySelector('table')?.remove();
  for(const f of source.fields){const row=d.createElement('tr'),label=d.createElement('td'),cell=d.createElement('td');label.textContent=f.label;let n=d.createElement(f.type==='select-one'?'select':'input');
   if(n.tagName==='SELECT'){for(const choice of source.options[f.index]||[{label:f.value}]){const option=d.createElement('option');option.textContent=choice.label;option.value='native:'+choice.label;n.append(option);}n.value='native:'+f.value;}
   else{n.type=f.type;n.value=f.value;if(f.checked!==null)n.checked=f.checked;if(f.type==='radio')n.name=stage+':'+(layout.priceIndices.includes(f.index)?'price':layout.signalIndices?.includes(f.index)?'signal':f.index>=7&&f.index<=10?'retracement':'volume');}
   n.disabled=f.disabled;cell.append(n);row.append(label,cell);table.append(row);
  }p.prepend(table);const current=nodes(p);
  for(const [gate,...children]of layout?.gates||[[4,5]]){const update=()=>children.forEach(i=>current[i].disabled=!current[gate].checked);current[gate].addEventListener('change',update);update();}
  if(layout){current[layout.chartIndex].addEventListener('change',()=>{const chart=current[layout.chartIndex].selectedOptions[0].textContent;w.setTimeout(()=>render(p,stage,chart),100);});
   if(chart==='Renko')current[layout.modeIndex].addEventListener('change',()=>{const value=current[layout.modeIndex].selectedOptions[0].textContent;nativeModes.push({stage,value});current[layout.sizeIndex].value=['ATR','ATR %'].includes(value)?'14':'1';});
  }
  if(stage==='momentum'){
   const group=current[1];group.placeholder='Search Group';group.addEventListener('keyup',()=>{menus.forEach(m=>m.remove());const menu=d.createElement('div');menu.className='popupContent';menu.innerHTML='<ul class="ind-list"></ul>';const li=d.createElement('li');li.setAttribute('grpid','group:demo-40');li.textContent='Demo universe 40';li.onclick=()=>{group.value=li.textContent;group.dataset.groupId=li.getAttribute('grpid');groupCommits++;menu.remove();};menu.firstChild.append(li);menus.push(menu);d.body.append(menu);});
  }
 }
 const runtime={id:'variant-test',getURL:p=>'chrome-extension://variant-test/'+p},source={id:runtime.id,url:w.location.href,tab:{id:9}},dashboard={id:runtime.id,url:runtime.getURL('index.html')},storage={get:async key=>reordered(E.clone(key?{[key]:memory[key]??null}:memory)),set:async value=>Object.assign(memory,E.clone(value))},coordinator=createCoordinator({storage,runtime,uuid:()=> 'variant-id-'+(++uid)});
 w.chrome={storage:{local:storage},runtime:{...runtime,sendMessage:message=>coordinator.handle(message,source),onMessage:{addListener:fn=>listeners.push(fn)}}};w.URL.createObjectURL=()=> 'blob:variant';w.URL.revokeObjectURL=()=>{};
 for(const file of ['core.js','presentation.js','intelligence.js','source-layouts.js','setup.js','experiments.js','capture.js'])w.eval(fs.readFileSync(path.join(base,file),'utf8'));
 const raw=S.demoTemplate({momentumChart:mainChart,executionChart:exitChart,...(mainChart==='Renko'?{momentumBrickMode:mainMode}:{}),...(exitChart==='Renko'?{executionBrickMode:exitMode}:{})});raw.demo=false;
 // Record the observed DOM's disablement, as a real source read does, before
 // resetting both forms to Candle to exercise chart transitions on every run.
 render(main,'momentum',mainChart,mainMode);raw.stages.momentum.fields=fields(main);raw.stages.momentum.options[1]=[{value:'Demo universe 40',label:'Demo universe 40',sourceValue:'group:demo-40'}];
 for(const stage of ['execution','portfolio']){const p=popup('Fixture settings');render(p,stage,exitChart,exitMode);raw.stages[stage].fields=fields(p);p.remove();}render(main,'momentum','Candle');
 const template=S.template(raw),config=S.defaults(template),mainLayout=L.main(mainChart),exitLayout=L.execution(exitChart),variantStage=mainChart!=='Candle'?'momentum':'execution',variantChart=variantStage==='momentum'?mainChart:exitChart,variantMode=variantStage==='momentum'?mainMode:exitMode,sizeKey=variantStage+(variantChart==='P&F'?'.box.size':'.brick.size'),sizes=variantChart==='P&F'?[0.25,0.5,1]:['ATR','ATR %'].includes(variantMode)?[7,14,21]:[0.5,1,2];
 for(const [stage,chart,layout]of [['momentum',mainChart,mainLayout],['execution',exitChart,exitLayout]])if(chart!=='Candle'){
  config[stage+'.price.close-only']=stage==='execution';config[stage+'.price.high-low']=stage==='momentum';
  if(chart==='P&F')config[stage+'.box.reversal']=stage==='momentum'?'5':'4';
 }
 if(mainChart!=='Candle'){config['momentum.signal-mode']='35';config['momentum.strategy.1.input']=0.75;}
 config['execution.target']=7;config['execution.stop']=4;config['execution.exit.enabled']=true;config['portfolio.capital']=250000;
 plan=E.create({id:'variant-proof',name:'Fictional '+mainChart+' / '+exitChart+' lifecycle',baseline:S.configToBaseline(config,template,{id:'variant-baseline',name:'Variant fixture',demo:false}),dimensions:[{key:sizeKey,values:sizes}],objective:'returns',minTrades:0});memory['experiment:'+plan.id]=plan;
 const done=d.createElement('span');done.textContent='BackTest Completed.';main.append(done);let backtest;
 backtest=button(main,'BackTest',()=>{const p=popup('Momentum Trading BackTest');render(p,'execution','Candle');button(p,'Backtest',()=>{
  const index=submissions.length,trial=plan.trials[index];if(index)durableBeforeNext.push(!!memory['run:'+plan.trials[index-1].runId]);
  assert.equal(nodes(main)[1].dataset.groupId,'group:demo-40','The native universe identity must be committed before submission.');
  submissions.push({momentum:fields(main),execution:fields(p)});E.verify(E.fields(E.expected(plan,trial),'momentum'),submissions[index].momentum);E.verify(E.fields(E.expected(plan,trial),'execution'),submissions[index].execution);
  backtest.textContent='Cancel BackTest';if(failure!=='stale')done.textContent='Processing';
  w.setTimeout(()=>{backtest.textContent='BackTest';done.textContent='BackTest Completed.';p.remove();},400);
  if(failure==='stale')w.setTimeout(()=>popup('Error'),650);
 });});
 button(main,'Portfolio Testing',()=>{const p=popup('Portfolio Backtesting');render(p,'portfolio');button(p,'Backtest',()=>{
  portfolioSubmissions.push(fields(p));if(failure==='duplicate'&&oldReport){d.body.append(oldReport);return;}
  const report=popup('Portfolio Backtesting Report');oldReport=report;reportCount++;const tabs=d.createElement('div');for(const name of ['Quick Stats','Statistics','Charts','Trade Details']){const tab=d.createElement('div');tab.setAttribute('role','tab');tab.textContent=name;tab.onclick=()=>{for(const t of tabs.children)t.className='';tab.className='selected';};tabs.append(tab);}report.append(tabs);
  const panel=d.createElement('div');panel.setAttribute('role','tabpanel');panel.innerHTML='<div class="stats-card"><div class="status">Total no. of Trades</div><div class="amt">4</div></div><div class="stats-card"><div class="status">Gross Total Returns( % )</div><div class="amt">'+reportCount+'%</div></div><table class="dropdown-table-body"><tr><td>Group</td><td>Demo universe 40</td></tr></table>'+Array.from({length:6},(_,i)=>'<svg class="highcharts-root" xmlns="http://www.w3.org/2000/svg"><title>Chart '+i+'</title><path d="M0 0 L20 10"/></svg>').join('')+'<table class="rade-result-detail"></table><span id="curPageTextEle">1</span><span id="lastPageTextEle">2</span><img src="/firstPage.png"><img src="/next.png">';report.append(panel);
  function page(n){panel.querySelector('#curPageTextEle').textContent=n;panel.querySelector('.rade-result-detail').innerHTML='<tr><th>Sr #</th><th>Symbol</th><th>Qty</th></tr>'+[n*2-1,n*2].map(i=>'<tr><td>'+i+'</td><td>FICTIONAL'+i+'</td><td>'+(i===2?0:10)+'</td></tr>').join('');}panel.querySelector('img[src$="firstPage.png"]').onclick=()=>page(1);panel.querySelector('img[src$="next.png"]').onclick=()=>page(2);page(1);
 });});
 w.eval(fs.readFileSync(path.join(base,'runner.js'),'utf8'));
 try{
  for(let n=0;n<30&&!memory['runner:tab:9']?.ready;n++)await sleep(50);
  const started=await coordinator.handle({action:'start',id:plan.id,tabId:9},dashboard);assert.equal(started.ok,true,JSON.stringify(started));for(const fn of listeners)fn({type:'vault-runner-wake'},{id:runtime.id},()=>{});
  for(let n=0;n<1200&&!['complete','needs-review'].includes(memory['experiment:'+plan.id].status);n++)await sleep(100);
  const result=memory['experiment:'+plan.id],runs=Object.entries(memory).filter(([key])=>key.startsWith('run:')).map(([,run])=>run);
  if(failure){assert.equal(result.status,'needs-review',JSON.stringify(result.trials));assert.equal(runs.length,failure==='duplicate'?1:0);assert.equal(submissions.length,failure==='duplicate'?2:1);assert.equal(portfolioSubmissions.length,failure==='duplicate'?2:0);if(failure==='duplicate')assert.match(result.trials[1].error,/confirmed submissions/);return;}
  assert.equal(result.status,'complete',JSON.stringify(result.trials.map(t=>({status:t.status,error:t.error}))));assert.equal(runs.length,3);assert.equal(submissions.length,3);assert.equal(portfolioSubmissions.length,3);assert.equal(groupCommits,3);assert.deepEqual(durableBeforeNext,[true,true]);
  for(const trial of result.trials){const run=memory['run:'+trial.runId],at=trial.ordinal-1;assert.ok(run);E.verifySettings(E.expected(plan,trial),run);for(const stage of ['momentum','execution'])assert.deepEqual(E.fields(run,stage),submissions[at][stage],'Saved '+stage+' controls must equal the actual submission snapshot.');assert.deepEqual(E.fields(run,'portfolio'),portfolioSubmissions[at]);assert.equal(run.provenance,'recorded-at-submit');assert.equal(run.trades.rows.length,4);assert.equal(run.trades.rows[1][2],'0');assert.equal(run.charts.length,6);
   assert.equal(E.fields(run,'momentum')[0].value,mainChart);assert.equal(E.fields(run,'execution')[3].value,exitChart);const sourceFields=E.fields(run,variantStage),sourceLayout=L.stage(variantStage,sourceFields);assert.equal(sourceFields[sourceLayout.sizeIndex].value,String(sizes[at]),'Each trial must apply and capture its requested box/brick size, after any native mode reset.');
   for(const [stage,chart,layout]of [['momentum',mainChart,mainLayout],['execution',exitChart,exitLayout]])if(chart!=='Candle'){const actual=E.fields(run,stage);assert.equal(actual[layout.priceIndices[0]].checked,stage==='execution');assert.equal(actual[layout.priceIndices[1]].checked,stage==='momentum');if(chart==='P&F')assert.equal(actual[layout.modeIndex].value,stage==='momentum'?'5':'4');else assert.equal(actual[layout.modeIndex].value,stage==='momentum'?mainMode:exitMode);}
   if(mainChart!=='Candle'){assert.equal(E.fields(run,'momentum')[35].checked,true);assert.equal(E.fields(run,'momentum')[34].checked,false);assert.equal(E.fields(run,'momentum')[mainLayout.rows[1].valueIndex].value,'0.75');}
   const evidence=run.experiment.evidence;assert.equal(evidence.sourceSession,memory['runner:tab:9'].session);assert.equal(evidence.strategySubmissionId,run.parameters.strategy.id);assert.equal(evidence.portfolioSubmissionId,run.parameters.id);const times=['strategySubmittedAt','strategyStartedAt','strategyCompletedAt','portfolioSubmittedAt','reportOpenedAt','capturedAt'].map(k=>Date.parse(evidence[k]));assert.ok(times.every((t,i)=>Number.isFinite(t)&&(!i||t>=times[i-1])));
  }
  ...
  for(const [stage,chart,mode]of [['momentum',mainChart,mainMode],['execution',exitChart,exitMode]])if(chart==='Renko'&&mode!=='Percent')assert.ok(nativeModes.some(item=>item.stage===stage&&item.value===mode),'Exercise native '+stage+' Renko mode reset before size assignment.');
 }finally{dom.window.close();}
}
async function backgroundFocusChecks(){
 ...
 for(const mode of ['return','source-error','user-switch','closed-vault','changed-vault','moved-vault','unrelated-tab','late-read','late-activation','rule-search','symbol-search','filter-load']){
  ...
  const chrome={storage:{local:{}},runtime:{id:'test-ext',getURL:file=>'chrome-extension://test-ext/'+file,onMessage:{addListener:()=>{}}},action:{onClicked:{addListener:()=>{}}},tabs:{
   ...
   sendMessage:async(id,message)=>{
    assert.equal(id,9);assert.equal(active,9);assert.equal(message.type,mode==='rule-search'?'vault-runner-rule-search':mode==='symbol-search'?'vault-runner-symbol-search':'vault-runner-config');reads++;
    if(mode==='symbol-search')assert.deepEqual({...message},{type:'vault-runner-symbol-search',stage:'momentum',fieldIndex:53,market:'NSE',query:'Fictional',session:'s'},'Internal routing hints must not leak into the source request.');
    if(mode==='rule-search')assert.deepEqual({...message},{type:'vault-runner-rule-search',stage:'marketFilter',parentIndex:19,category:'Public',query:'Fictional',session:'s'});
    if(mode==='filter-load')assert.deepEqual({...message,changes:{...message.changes}},{type:'vault-runner-config',changes:{},loadFilter:'relativeStrength'});
  ...
  context.importScripts=(...files)=>files.forEach(file=>vm.runInContext(fs.readFileSync(path.join(base,file),'utf8'),context,{filename:file}));
  vm.runInContext(fs.readFileSync(path.join(base,'background.js'),'utf8'),context,{filename:'background.js'});
  const lookup=mode==='symbol-search'?{kind:'symbol',stage:'momentum',fieldIndex:53,market:'NSE',query:'Fictional',session:'s'}:mode==='rule-search'?{stage:'marketFilter',parentIndex:19,category:'Public',query:'Fictional',session:'s'}:undefined;
  const result=context.configure(9,{},lookup,mode==='filter-load'?{loadFilter:'relativeStrength'}:{});
  ...
  if(['return','source-error','rule-search','symbol-search','filter-load'].includes(mode))assert.deepEqual(actions,[9,10]);
...
async function chartScenario(chartCase,dailySeed){
 ...
 const initial=chartCase==='Renko'?'Renko':['warm-reversal','exit-context'].includes(chartCase)?'P&F':'Candle',daily=['daily-categories','saved-visited-layout'].includes(chartCase);let executionState=null,privateRadarChoices=chartCase==='cache-reconnect'||daily?['Fictional private radar']:[];const categoryRequests=[],groupRows=['Demo universe 40','Other fictional group'];
 ...
 function render(p,stage,descriptor){
  ...
  const dailyLabels=()=>{if(!daily)return;for(const row of layout.rows){const name=row.name==='Radar'?'Radar':stage==='execution'?'Exit strategy':'Str '+row.name.at(-1),own=name+' : '+current[row.parentIndex].selectedOptions[0].textContent,prefix=stage==='momentum'&&row.name==='Strategy 3'?'Str 2 : '+current[layout.rows[2].parentIndex].selectedOptions[0].textContent+' → ':'';for(const index of [row.parentIndex,row.childIndex,row.valueIndex,row.gateIndex].filter(Number.isInteger))current[index].closest('tr').firstElementChild.textContent=prefix+own;}};
  for(const row of layout.rows){
   let child=current[row.childIndex];const populate=()=>{const category=current[row.parentIndex].selectedOptions[0].textContent,search=row.name!=='Radar'&&['My','Public'].includes(category),next=d.createElement(search?'input':'select');next.disabled=!current[row.gateIndex].checked;child.replaceWith(next);child=next;current[row.childIndex]=next;
    if(search){next.type='text';next.placeholder='Search System Builder';next.addEventListener('keyup',()=>{for(const menu of menus)menu.remove();if(!next.value)return;const menu=d.createElement('div');menu.className='popupContent';menus.add(menu);if(category==='My'||chartCase==='exit-context'&&next.value==='missing')menu.innerHTML='<div class="gwt-HTML">No matching system builder found</div>';else{const ul=d.createElement('ul');ul.className='ind-list';const li=d.createElement('li');li.setAttribute('sbid',stage+':'+layout.chart+':public');li.textContent=layout.chart+(chartCase==='exit-context'?' double rule':' public rule');li.onclick=()=>{next.value=li.textContent;menu.remove();};ul.append(li);menu.append(ul);}d.body.append(menu);});}
 ...
 try{
  ...
  if(chartCase==='exit-context'){
   // This routing/restoration check uses validated fictional daily menus so
   // it does not repeat unrelated catalogue discovery before the actual test.
   const cacheStage=(p,stage)=>{const fields=w.VaultCapture.fields(p),layout=L.stage(stage,fields),controls=nodes(p),option=value=>({value,label:value,sourceValue:'native:'+value,disabled:false}),nativeOptions=Object.fromEntries(controls.flatMap((node,index)=>node.tagName==='SELECT'?[[index,[...node.options].map(item=>({value:item.textContent,label:item.textContent,sourceValue:item.value,disabled:item.disabled}))]]:[])),ruleCatalogues={};
    for(const row of layout.rows){const categories={},controlTypes={},fieldLabels={},indices=[row.parentIndex,row.childIndex,...(Number.isInteger(row.valueIndex)?[row.valueIndex]:[]),row.gateIndex];for(const category of nativeOptions[row.parentIndex].map(option=>option.label)){const search=row.name!=='Radar'&&['My','Public'].includes(category);controlTypes[category]=search?'text':'select-one';fieldLabels[category]=indices.map(index=>fields[index].label);categories[category]=search||category==='My'?[]:category===fields[row.parentIndex].value?nativeOptions[row.childIndex]:[...new Set([fields[row.childIndex].value,layout.chart+' '+row.name+' '+category+' rule'])].map(option);}ruleCatalogues[row.childIndex]={parentIndex:row.parentIndex,gateIndex:row.gateIndex,categories,controlTypes,fieldLabels,searchQueries:{}};}
    return {context:{chart:layout.chart,...(stage==='momentum'?{market:fields[layout.marketIndex].value,relativeStrength:false}:{selection:fields[layout.selectionIndex].value}),...(Number.isInteger(layout.modeIndex)?{mode:fields[layout.modeIndex].value}:{}),categories:layout.rows.map(row=>[row.parentIndex,fields[row.parentIndex].value])},signature:fields.map(field=>[field.type,field.label]),nativeOptions,ruleCatalogues,...(stage==='momentum'?{groupOptions:groupRows.map(label=>({value:label,label,sourceValue:'group:'+label,disabled:false}))}:{})};};
   const p=openExecution();exitSeed={schemaVersion:2,adapterVersion:L.version,daily:true,stages:{momentum:cacheStage(main,'momentum'),execution:cacheStage(p,'execution')}};p.querySelector('.close-buton').click();
  }
  if(chartCase==='daily-passive-state'){const extra=d.createElement('option');extra.value='native:Monthly';extra.textContent='Monthly';nodes(main)[41].append(extra);}
  const before=JSON.stringify(w.VaultCapture.fields(main)),first=await request(dailySeed?{cachedChoices:dailySeed.records}:exitSeed?{cachedChoices:[exitSeed]}:{});assert.equal(first.ok,true,first.error);assert.equal(JSON.stringify(w.VaultCapture.fields(main)),before);assert.equal(first.choicesFromCache,!!(dailySeed||exitSeed));assert.equal(first.choiceCache.adapterVersion,L.version);assert.equal(first.choiceCache.stages.momentum.context.chart,initial);assert.equal(JSON.stringify(first.choiceCache).includes('capturedAt'),false);assert.equal(Object.hasOwn(first.choiceCache.stages.momentum,'fields'),false);
  if(chartCase==='saved-visited-layout'){
   const tooltip=' / iThis feature can help you to look at combined result of the group of stocks you have backtested. It assumes equal amount of allocation in all stocks and calculates the total performance of the stocks in the group. The average return, hit ratio and risk-reward ratio is calculated on all stocks put together gives idea about the performance of systems tested on group of stocks. For equity curve & drawdown, progressive P&L is calculated on the allocated amount.';
   let records=await persistedMenus([first]);
   const adorn=(on)=>{const f=w.VaultCapture.fields(main),layout=L.stage('momentum',f),controls=nodes(main);for(const field of f){const cell=controls[field.index].closest('tr').firstElementChild;let label=cell.textContent.split(tooltip).join('').replace(/^RS SB : \/ (?:Pre|My|Public|Popular)i → /,'');if(field.index===layout.rsIndex)label=label.replace(/ \/ NSE$/,'');if(on){if(field.index===layout.rsIndex)label+=' / NSE';if(layout.variant&&[layout.sizeIndex,layout.modeIndex].includes(field.index))label='RS SB : / Prei → '+label;if([...Array(8)].some((_,n)=>field.index===layout.rows[2].parentIndex+n))label=label.replace(/(Str 2 : (?:Pre|My|Public|Popular))(?= → |$)/,'$1'+tooltip);}cell.textContent=label;}};
   for(const chart of ['Candle','P&F','Renko']){
    const fresh=chart==='Candle'?first:await request({changes:{momentum:{0:chart}},cachedChoices:records});assert.equal(fresh.ok,true,fresh.error);records=await persistedMenus([first,fresh]);
    adorn(true);const beforeCounts={...counts},beforeFields=JSON.stringify(w.VaultCapture.fields(main)),visited=await request({cachedChoices:records});assert.equal(visited.ok,true,visited.error);assert.equal(visited.choicesFromCache,true);assert.deepEqual(counts,beforeCounts,'Opening hidden RS/help markup must not rediscover menus');assert.equal(JSON.stringify(w.VaultCapture.fields(main)),beforeFields);assert.doesNotThrow(()=>S.template(visited.config));
    const expected=S.defaults(fresh.config);expected['momentum.period.1']=500;expected['portfolio.capital']=450000;const saved=structuredClone(D.create()[0]);saved.demo=false;saved.parameters=S.configToBaseline(expected,fresh.config,{id:'visited-saved',name:'Fictional saved settings',demo:false}).parameters;const immutable=JSON.stringify(saved);assert.deepEqual(S.configFromRun(saved,visited.config),expected,'Saved inputs copy into the visited form exactly');assert.equal(JSON.stringify(saved),immutable);
    const visitedRecords=await persistedMenus([visited]);adorn(false);const restored=await request({cachedChoices:visitedRecords});assert.equal(restored.ok,true,restored.error);assert.equal(restored.choicesFromCache,true);assert.deepEqual(counts,beforeCounts,'Existing decorated cache works on a fresh clean form');assert.doesNotThrow(()=>S.template(restored.config));
    const label=nodes(main)[12].closest('tr').firstElementChild;label.textContent='Period : changed meaning';const rejected=await request({cachedChoices:visitedRecords});assert.equal(rejected.ok,false);assert.match(rejected.error,/layout/);assert.deepEqual(counts,beforeCounts,'A meaningful label change still stops without a rescan');label.textContent='Period :';
    if(chart==='Candle'){
     const row=L.main(chart).rows[2],gate=nodes(main)[row.gateIndex],enabled=gate.checked;if(!enabled)gate.click();const parent=nodes(main)[row.parentIndex];parent.value='native:My';parent.dispatchEvent(new w.Event('change',{bubbles:true}));await sleep(200);adorn(true);const changedCounts={...counts},changed=await request({cachedChoices:visitedRecords});assert.equal(changed.ok,true,changed.error);assert.doesNotThrow(()=>S.template(changed.config));assert.deepEqual(counts,changedCounts,'Category and help-label projection compose without discovery');for(const labels of Object.values(changed.config.stages.momentum.ruleCatalogues[48].fieldLabels))assert.ok(labels.every(value=>value.startsWith('Str 2 : My'+tooltip+' → Str 3 :')),'STR3 keeps the current STR2 category and complete raw help prefix');parent.value='native:Pre';parent.dispatchEvent(new w.Event('change',{bubbles:true}));await sleep(200);if(!enabled)gate.click();adorn(false);
    }
    records=await persistedMenus([first,fresh,restored]);
   }
   const beforeSwitch={...counts},candle=await request({changes:{momentum:{0:'Candle'}},cachedChoices:records});assert.equal(candle.ok,true,candle.error);assert.equal(candle.choicesFromCache,true);assert.deepEqual(counts,beforeSwitch,'Saved-context chart switching reuses the existing Candle choices');assert.equal(counts.submissions,0);return;
  }
  if(chartCase==='daily-passive-state'){
   const F=require('./fixtures/filter-setup.cjs'),indexed=S.template(F({marketChart:'P&F',exitPrices:true})).stages.marketFilter,variantFilter=S.template(F({marketChart:'P&F',exitPrices:false})).stages.marketFilter,cacheFilter=descriptor=>{descriptor=structuredClone(descriptor);descriptor.ruleCatalogues=Object.fromEntries(L.marketFilter(descriptor.fields).rows.map(row=>[row.childIndex,{parentIndex:row.parentIndex,gateIndex:row.gateIndex,categories:{Pre:descriptor.options[row.childIndex]},controlTypes:{Pre:'select-one'},fieldLabels:{Pre:[row.parentIndex,row.childIndex,row.gateIndex].map(index=>descriptor.fields[index].label)}}]));return {schemaVersion:2,adapterVersion:L.version,daily:true,stages:{marketFilter:w.VaultCacheFixture.choiceStageCache('marketFilter',descriptor)}};};
   const rs=structuredClone(indexed),filterLayout=L.marketFilter(rs.fields);rs.fields[filterLayout.indexModeIndex].checked=false;rs.fields[filterLayout.rsModeIndex].checked=true;assert.equal(w.VaultCacheFixture.cachedStage('marketFilter',rs,[cacheFilter(indexed)],false),null,'P&F Index and RS remain independent menu contexts.');
   const matched=w.VaultCacheFixture.cachedStage('marketFilter',indexed,[cacheFilter(indexed),cacheFilter(variantFilter)],false);assert.equal(matched.signature.length,indexed.fields.length,'A newer known MTF layout variant cannot mask the matching saved layout.');
   const records=await persistedMenus([first]),prior={...counts};
   // Native menu ordering is presentation, not a changed set of identities.
   for(const index of [0,40]){const menu=nodes(main)[index],selected=menu.value;for(const option of [...menu.options].reverse())menu.append(option);menu.value=selected;}
   let read=await request({cachedChoices:records});assert.equal(read.ok,true,read.error);assert.equal(read.choicesFromCache,true);assert.deepEqual(counts,prior,'Reordered options reuse today\'s choices without discovery.');
   // An inactive selector can expose only its current option after a fresh
   // render. That must neither reject nor overwrite its complete daily menu.
   const gate=nodes(main)[42];if(gate.checked)gate.click();nodes(main)[41].querySelector('option[value="native:Monthly"]').remove();
   read=await request({cachedChoices:records});assert.equal(read.ok,true,read.error);assert.equal(read.config.stages.momentum.fields[42].checked,false);assert.ok(read.config.stages.momentum.options[41].some(option=>option.label==='Monthly'));assert.deepEqual(counts,prior);
   const retained=await persistedMenus([read]),extra=d.createElement('option');extra.value='native:Monthly';extra.textContent='Monthly';nodes(main)[41].append(extra);gate.click();
   read=await request({cachedChoices:retained});assert.equal(read.ok,true,read.error);assert.equal(read.choicesFromCache,true);assert.deepEqual(counts,prior,'Off → persisted → On must still reuse the complete menu.');
   // Same-context layout variants coexist in storage. A newer incompatible
   // variant cannot mask an older exact match or trigger a scan.
   const variant=structuredClone(retained.find(record=>record.stages.momentum));variant.stages.momentum.signature[12][1]+=' variant';
   read=await request({cachedChoices:[...retained,variant]});assert.equal(read.ok,true,read.error);assert.equal(read.choicesFromCache,true);assert.deepEqual(counts,prior);
   const saved=JSON.stringify(w.VaultCapture.fields(main)),bad=structuredClone(retained);bad.find(record=>record.stages.momentum).stages.momentum.nativeOptions[0][0].sourceValue+=' changed';
   const rejected=await request({cachedChoices:bad});assert.equal(rejected.ok,false,'Changed active native identities remain rejected.');assert.match(rejected.error,/Recheck all choices/);assert.deepEqual(counts,prior);assert.equal(JSON.stringify(w.VaultCapture.fields(main)),saved);assert.equal(w.VaultCapture.popup('Momentum Trading BackTest'),undefined);
   const pnf=await request({changes:{momentum:{0:'P&F'}},cachedChoices:retained});assert.equal(pnf.ok,true,pnf.error);const pnfRecords=await persistedMenus([read,pnf]),reversal=nodes(main)[L.main('P&F').modeIndex];assert.ok([...reversal.options].some(o=>o.textContent==='5'));reversal.value='native:5';const pnfCounts={...counts};
   const changedReversal=await request({cachedChoices:pnfRecords});assert.equal(changedReversal.ok,true,changedReversal.error);assert.equal(changedReversal.choicesFromCache,true);assert.equal(changedReversal.config.stages.momentum.fields[L.main('P&F').modeIndex].value,'5');assert.deepEqual(counts,pnfCounts,'P&F reversal is a setting; it does not require downloading its chart menus again.');return;
  }
  if(chartCase==='exit-context'){
   const mixed=first;assert.equal(counts.categories,0,'The lookup regression must not rescan unrelated rule catalogues.');assert.equal(mixed.config.stages.momentum.fields[0].value,'P&F');assert.equal(mixed.config.stages.execution.fields[3].value,'Candle');
   // RZone rebuilds its settings dialog from the main chart when reopened.
   // Vault's independently staged execution chart must own the lookup route.
   executionState=state('execution','P&F');executionState.fields[1].value='2022-01-01';executionState.fields[5].value='5';const native=openExecution(),sourceBefore=JSON.stringify(w.VaultCapture.fields(native));native.querySelector('.close-buton').click();const mainBefore=JSON.stringify(w.VaultCapture.fields(main)),exit=L.execution('Candle').rows[0];
   for(const query of ['double','missing']){const found=await request({type:'vault-runner-rule-search',stage:'execution',parentIndex:exit.parentIndex,category:'Public',query,session:mixed.session});assert.equal(found.ok,true,found.error);assert.equal(found.result.stage,'execution');assert.equal(found.result.parentIndex,exit.parentIndex);assert.equal(found.result.childIndex,exit.childIndex);if(query==='double'){assert.deepEqual(JSON.parse(JSON.stringify(found.result.options)),[{value:'Candle double rule',label:'Candle double rule',sourceValue:'execution:Candle:public',disabled:false}]);assert.equal(found.result.choiceCache.stages.execution.context.chart,'Candle');}else assert.equal(found.result.options.length,0,'An exact native no-match still returns a successful empty lookup.');assert.equal(JSON.stringify(w.VaultCapture.fields(main)),mainBefore,'Lookup never changes the independent main P&F settings.');assert.equal(JSON.stringify(executionState.fields),sourceBefore,'The original P&F execution chart, dates, reversal and remaining controls are restored.');assert.equal(w.VaultCapture.popup('Momentum Trading BackTest'),undefined);assert.equal([...menus].filter(menu=>menu.isConnected).length,0);assert.equal(counts.submissions,0);}
   return;
  }
  ...
  if(chartCase==='cache-reconnect'||daily){
   ...
   if(!daily)return;
   const source=nodes(main);source[46].click();source[43].value='native:My';source[43].dispatchEvent(new w.Event('change',{bubbles:true}));await sleep(200);const changedCounts={...counts},changed=await request({cachedChoices:dailySeed.records});assert.equal(changed.ok,true,changed.error);assert.equal(changed.choicesFromCache,true);assert.deepEqual(counts,changedCounts,'Changing only a native rule category reuses observed menus.');assert.equal(changed.config.stages.momentum.fields[44].type,'text');assert.ok(changed.config.stages.momentum.fields[47].label.startsWith('Str 2 : My → '));for(const labels of Object.values(changed.config.stages.momentum.ruleCatalogues[48].fieldLabels))assert.ok(labels.every(label=>label.startsWith('Str 2 : My → ')),'Every retained STR3 category must follow the current STR2 prefix.');assert.doesNotThrow(()=>S.template(changed.config));assert.equal(nodes(main)[46].checked,true,'Cache reuse keeps the current source gate.');
   const beforeForce={...counts},forced=await request({cachedChoices:dailySeed.records,forceChoices:true});assert.equal(forced.ok,true,forced.error);assert.equal(forced.choicesFromCache,false);assert.ok(counts.groups>beforeForce.groups&&counts.categories>beforeForce.categories,'Manual refresh bypasses full daily metadata.');
   const bad=structuredClone(forced.choiceCache);delete bad.session;bad.daily=true;bad.stages.momentum.signature[12][1]+=' changed';const prior={...counts},rejected=await request({cachedChoices:[bad]});assert.equal(rejected.ok,false);assert.match(rejected.error,/Recheck all choices/);assert.deepEqual(counts,prior,'Contradictory same-day metadata stops without a full automatic scan.');assert.equal(w.VaultCapture.popup('Momentum Trading BackTest'),undefined);return;
  }
  if(chartCase==='cache'){
...
async function filterScenario(mode,dailySeed){
 const variantRS=mode==='relative-strength-pnf'||mode==='relative-strength-renko',rsChart=mode==='relative-strength-pnf'?'P&F':'Renko';
 const L=require('../dist/source-layouts.js'),F=require('./fixtures/filter-setup.cjs'),sourceTemplate=F(variantRS?{chart:rsChart}:['market-renko','market-renko-delayed'].includes(mode)?{marketChart:'Renko',exitPrices:true}:{}),dom=new JSDOM('<body><h1 class="header-text">Momentum Trading BackTesting</h1><div class="account-right"></div></body>',{runScripts:'outside-only',url:'https://zone.definedgesecurities.com/index.html#research'}),w=dom.window,d=w.document,main=d.querySelector('.account-right'),listeners=[],memory={},menus=new Set(),commits=[];
 installFixtureTime(w);
 ...
 const symbolRoles=['benchmarkIndex','indexSymbolIndex','numeratorSymbolIndex','denominatorSymbolIndex'],symbolSelections=new Map();for(const [stage,raw]of [['momentum',rsState],['marketFilter',filterFull]]){const layout=L.stage(stage,raw);for(const role of symbolRoles)if(Number.isInteger(layout[role]))symbolSelections.set(stage+':'+role,raw[layout[role]].value+'|'+stage+':'+role);}
 const selectionIDs=()=>JSON.stringify([...symbolSelections].sort(([a],[b])=>a.localeCompare(b))),originalSelectionIDs=selectionIDs(),noSymbolMenus=()=>assert.equal([...d.querySelectorAll('[data-symbol-menu],.tool-popup')].filter(menu=>menu.isConnected).length,0,'No symbol popup may remain after its source operation or parent dialog closes.'),closeSymbolMenu=menu=>(menu.parentElement?.classList.contains('tool-popup')?menu.parentElement:menu).remove();
 const nodes=p=>[...p.querySelectorAll('input,select')].filter(n=>n.type!=='hidden'),clearMenus=()=>{for(const menu of menus)if(!menu.hasAttribute('data-symbol-menu'))menu.remove();},fields=p=>JSON.parse(JSON.stringify(w.VaultCapture.fields(p)));
 const labels=p=>nodes(p).map(n=>n.tagName==='SELECT'?n.selectedOptions[0]?.textContent:n.value),button=(p,text,fn)=>{const b=d.createElement('button');b.textContent=text;b.onclick=fn;p.append(b);return b;};
 const descriptor=(stage,layout)=>stage==='momentum'?(layout.relativeStrength?sourceTemplate.stages.momentum.relativeStrength:sourceTemplate.stages.momentum):sourceTemplate.stages[stage];
 function canonicalMarketIndex(layout,index){const full=L.marketFilter(sourceTemplate.stages.marketFilter.fields);for(const key of ['chartIndex','sizeIndex','modeIndex','indexModeIndex','rsModeIndex','indexMarketIndex','indexSymbolIndex','numeratorMarketIndex','numeratorSymbolIndex','denominatorMarketIndex','denominatorSymbolIndex','actionIndex','targetGateIndex','targetValueIndex','stopGateIndex','stopValueIndex','exitSizeIndex','exitModeIndex'])if(layout[key]===index)return full[key];for(const key of ['priceIndices','methodIndices','methodValueIndices','exitPriceIndices']){const n=layout[key].indexOf(index);if(n!==-1)return full[key][n];}for(const key of ['gateIndex','parentIndex','childIndex'])if(layout.rows[0]?.[key]===index)return full.rows[0][key];throw Error('Unknown fictional market index '+index);}
 function rememberFilter(raw,layout){const full=L.marketFilter(filterFull);for(const field of raw){const index=canonicalMarketIndex(layout,field.index);filterFull[index]={...field,index};}filterFull[full.actionIndex].value=L.marketActions[2];}
 function options(stage,layout,index,field){const row=layout?.rows.find(row=>row.parentIndex===index),base=descriptor(stage,layout);if(row)return row.name==='Radar'?['Pre','My']:['Pre','My','Public','Popular'];if(index===layout?.chartIndex)return [field.value];if(variantRS&&field.value==='Relative Strength popular')return [field.value];const known=base.options?.[stage==='marketFilter'?canonicalMarketIndex(layout,index):index];return known?.map(o=>typeof o==='string'?o:o.label)||[field.value];}
 function popup(title){const p=d.createElement('div');p.className='popupContent';const head=d.createElement('div');head.className='custom-dialog-header';const caption=d.createElement('div');caption.className='caption';caption.textContent=title;caption.onclick=clearMenus;const close=d.createElement('a');close.className='close-buton';close.onclick=()=>p.remove();head.append(caption,close);p.append(head);d.body.append(p);return p;}
 function render(p,stage,snapshot){
  p.querySelector('table')?.remove();const layout=stage==='portfolio'?null:L.stage(stage,snapshot),table=d.createElement('table');
  for(const field of snapshot){const row=d.createElement('tr'),label=d.createElement('td'),cell=d.createElement('td');label.textContent=field.label;const n=d.createElement(field.type==='select-one'?'select':'input');if(n.tagName==='SELECT'){for(const value of new Set(options(stage,layout,field.index,field))){const option=d.createElement('option');option.value='native:'+value;option.textContent=value;n.append(option);}n.value='native:'+field.value;}else{n.type=field.type;n.value=field.value;if(field.checked!==null)n.checked=field.checked;if(field.type==='radio')n.name=stage+':'+(stage==='marketFilter'?(layout.methodIndices.includes(field.index)?'method':layout.priceIndices.includes(field.index)?'top-price':layout.exitPriceIndices.includes(field.index)?'exit-price':'filter-mode'):layout?.priceIndices.includes(field.index)?'price':layout?.signalIndices?.includes(field.index)?'signal':/52 Week/.test(field.label)?'retracement':'volume');}n.disabled=field.disabled;cell.append(n);row.append(label,cell);table.append(row);}p.prepend(table);const current=nodes(p);
  current[layout?.chartIndex??0].closest('tr').firstElementChild.textContent=stage==='marketFilter'?'Chart Type :':snapshot[layout?.chartIndex??0].label;current[layout?.chartIndex??0].closest('tr').firstElementChild.onclick=clearMenus;
  const gates=layout?.gates||[[4,5]];for(const [gate,...children]of gates){const update=()=>children.forEach(i=>{if(current[i])current[i].disabled=!current[gate].checked;});current[gate].addEventListener('change',update);update();}
  for(const row of layout?.rows||[]){let child=current[row.childIndex];const before=variantRS&&row.name==='Relative Strength'?sourceTemplate.stages.momentum.relativeStrength.options[row.childIndex].map(o=>typeof o==='string'?o:o.label):options(stage,layout,row.childIndex,snapshot[row.childIndex]),populate=()=>{const category=current[row.parentIndex].selectedOptions[0].textContent,search=row.name!=='Radar'&&['My','Public'].includes(category),next=d.createElement(search?'input':'select');next.disabled=!current[row.gateIndex].checked;child.replaceWith(next);child=next;current[row.childIndex]=next;if(search){next.type='text';next.placeholder='Search System Builder';next.addEventListener('keyup',()=>{clearMenus();if(!next.value)return;const menu=d.createElement('div');menu.className='popupContent';menu.innerHTML='<div class="gwt-HTML">No matching system builder found</div>';menus.add(menu);d.body.append(menu);});}else{const offered=category==='My'?[]:category==='Pre'?before:[row.name+' popular'];for(const value of offered){const option=d.createElement('option');option.value='native:'+value;option.textContent=value;next.append(option);}}if(variantRS&&row.name==='Relative Strength'){const prefix='RS SB : / '+category+'i';for(const at of [row.parentIndex,row.childIndex])current[at].closest('tr').firstElementChild.textContent=prefix;for(const at of [layout.sizeIndex,layout.modeIndex])current[at].closest('tr').firstElementChild.textContent=prefix+' → '+sourceTemplate.stages.momentum.relativeStrength.fields[at].label;}};current[row.parentIndex].addEventListener('change',()=>{scans.categories++;w.setTimeout(populate,50);});}
  const symbolIndices=layout?['benchmarkIndex','indexSymbolIndex','numeratorSymbolIndex','denominatorSymbolIndex'].filter(key=>Number.isInteger(layout[key])).map(key=>layout[key]):[];
  for(const index of symbolIndices){
   const n=current[index],owner=stage+':'+symbolRoles.find(key=>layout[key]===index);n.placeholder='Search Symbol';if(symbolSelections.get(owner)?.split('|')[0]===n.value)n.dataset.symbol=symbolSelections.get(owner);
   const query=()=>{
    clearMenus();const existing=[...menus].filter(menu=>menu.isConnected&&menu.dataset.symbolMenu===owner);
    if(!n.value){const retained=symbolSelections.get(owner),late=mode==='market-late-symbol'&&existing.length&&!lateSymbolOwners.has(owner)?existing[0]:null;if(late)lateSymbolOwners.add(owner);for(const menu of existing)closeSymbolMenu(menu);if(existing.length)symbolQueryClears++;assert.equal(symbolSelections.get(owner),retained,'Clearing a transient query does not choose or change a source symbol.');
     if(late){const reused=lateSymbolOwners.size%2===1,pending=reused?late:late.cloneNode(true);nativeTimeout(()=>{const wrapper=d.createElement('div');wrapper.className='tool-popup';if(!pending.querySelector(':scope > .abcd-1')){const content=d.createElement('div');content.className='abcd-1';while(pending.firstChild)content.append(pending.firstChild);pending.append(content);}wrapper.append(pending);menus.add(pending);d.body.append(wrapper);lateSymbolReopens++;},200);}return;}

    scans.symbols++;
    if(missingSymbols){const wrapper=d.createElement('div'),menu=d.createElement('div');wrapper.className='tool-popup';menu.className='popupContent';menu.dataset.symbolMenu=owner;menu.innerHTML='<div class="abcd-1"><div><div><div class="gwt-HTML">No matching symbol found</div></div></div></div>';wrapper.append(menu);menus.add(menu);d.body.append(wrapper);noMatchLists++;return;}
    if(mode==='market-unknown-dialog'){popup('Unexpected source notice');return;}
    const menu=existing[0]||d.createElement('div');menu.replaceChildren();menu.className='popupContent';menu.dataset.symbolMenu=owner;const list=d.createElement('ul');list.className='ind-list';menus.add(menu);
    const exchange=changeSymbolExchange?'BSE':current[index-1].selectedOptions[0].textContent==='All'?'NSE':current[index-1].selectedOptions[0].textContent,label=n.value,id=label+'|'+(changeSymbolIdentity?'changed:':'')+owner,li=d.createElement('li');li.setAttribute('name',id);li.setAttribute('exhg',exchange);const select=d.createElement('div');select.className='symbol-search-list';const name=d.createElement('div');name.textContent=label;select.append(name);select.onclick=()=>{n.value=label;n.dataset.symbol=id;symbolSelections.set(owner,id);commits.push({stage,index,label,id,exchange});closeSymbolMenu(menu);};li.append(select);
    for(const side of ['buy','sell']){const trade=button(li,side,()=>{orderClicks++;throw Error('A trading action must never be used for symbol selection.');});trade.className='btn-'+side;}list.append(li);
    if(mode==='symbol-catalogue'&&n.value==='N'){
     list.replaceChildren();for(let at=0;at<200;at++){const label='N catalogue '+String(at).padStart(3,'0'),id=label+'|catalogue:'+at,row=d.createElement('li'),select=d.createElement('div'),name=d.createElement('div');row.setAttribute('name',id);row.setAttribute('exhg','NSE');select.className='symbol-search-list';name.textContent=label;select.append(name);select.onclick=()=>{n.value=label;n.dataset.symbol=id;symbolSelections.set(owner,id);commits.push({stage,index,label,id,exchange:'NSE'});closeSymbolMenu(menu);};row.append(select);list.append(row);}
    }
    if(['market-delayed-symbol','market-symbol-timeout'].includes(mode)){
     const shell=d.createElement('div'),spinner=d.createElement('div');shell.className='abcd-1';spinner.className='loading';menu.append(shell);pendingSymbolShells++;
     if(mode==='market-delayed-symbol'&&pendingSymbolShells%2===0){const stale=list.cloneNode(true);stale.querySelector('li').setAttribute('name',label+'|stale:'+owner);shell.append(stale);staleSymbolLists++;nativeTimeout(()=>{if(menu.isConnected&&n.value===label)shell.replaceChildren(spinner);},180);}else shell.append(spinner);
     if(!menu.isConnected){const wrapper=d.createElement('div');wrapper.className='tool-popup';wrapper.append(menu);d.body.append(wrapper);}if(mode!=='market-symbol-timeout')nativeTimeout(()=>{if(menu.isConnected&&n.value===label)shell.replaceChildren(list);},420);
    }else{menu.append(list);if(!menu.isConnected)d.body.append(menu);}
   };
   n.addEventListener('keyup',()=>w.setTimeout(query,30));current[index-1].addEventListener('change',()=>{n.value='';delete n.dataset.symbol;symbolSelections.delete(owner);});
  }
  if(stage==='momentum'){
   current[1].placeholder='Search Group';current[1].addEventListener('keyup',()=>{scans.groups++;clearMenus();const menu=d.createElement('div');menu.className='popupContent';const list=d.createElement('ul');list.className='ind-list';const offered=['Demo universe 40','Demo universe 20','Demo universe 60'].filter(label=>!current[1].value||label.toLowerCase().includes(current[1].value.toLowerCase()));for(const [index,label]of offered.entries()){const li=d.createElement('li');li.setAttribute('grpid','group:'+label);li.textContent=label;li.onclick=()=>{current[1].value=label;groupCommits++;menu.remove();};list.append(li);}menu.append(list);menus.add(menu);d.body.append(menu);});
   current[layout.mtfIndex].addEventListener('change',()=>{marketButton.disabled=!current[layout.mtfIndex].checked;});
   if(variantRS){const prefix=layout.relativeStrength?'RS SB : / '+current[layout.rows.at(-1).parentIndex].selectedOptions[0].textContent+'i':'RS SB : / Publici';if(layout.relativeStrength)for(const at of [layout.rows.at(-1).parentIndex,layout.rows.at(-1).childIndex])current[at].closest('tr').firstElementChild.textContent=prefix;for(const at of [layout.sizeIndex,layout.modeIndex])current[at].closest('tr').firstElementChild.textContent=prefix+' → '+snapshot[at].label.replace(/^RS SB : \/ (?:Pre|My|Public|Popular)i → /,'');}
   current[layout.rsIndex].addEventListener('change',()=>{const raw=fields(main),on=current[layout.rsIndex].checked;if(on){const next=E.clone(rsState);for(let index=0;index<=layout.rsIndex;index++)next[index]={...raw[index]};if(layout.variant){const expanded=L.main(layout.chart,true);for(const key of ['sizeIndex','modeIndex'])next[expanded[key]]={...raw[layout[key]],index:expanded[key]};layout.priceIndices.forEach((at,n)=>next[expanded.priceIndices[n]]={...raw[at],index:expanded.priceIndices[n]});}render(main,'momentum',next);}else{rsState=raw;const next=raw.slice(0,layout.rsIndex+1);if(layout.variant)for(const at of [layout.sizeIndex,layout.modeIndex,...layout.priceIndices])next.push({...raw[at],index:next.length});render(main,'momentum',next);}marketButton.disabled=!nodes(main)[2].checked;});
  }
  if(stage==='marketFilter'){
   const applyDisabled=()=>{const index=current[layout.indexModeIndex].checked;current[layout.indexSymbolIndex].disabled=!index;current[layout.numeratorSymbolIndex].disabled=index;current[layout.denominatorSymbolIndex].disabled=index;layout.methodValueIndices.forEach((at,n)=>current[at].disabled=!current[layout.methodIndices[n]].checked);};applyDisabled();
   for(const at of layout.methodIndices)current[at].addEventListener('change',applyDisabled);
   const rebuild=()=>{const raw=fields(p),action=raw[layout.actionIndex].value;rememberFilter(raw,layout);render(p,'marketFilter',L.projectMarketFilter(filterFull,{mode:raw[layout.rsModeIndex].checked?'RS':'Index',action}));};
   for(const at of [layout.indexModeIndex,layout.rsModeIndex])current[at].addEventListener('change',layout.variant?rebuild:applyDisabled);
   current[layout.actionIndex].addEventListener('change',rebuild);
   if(layout.chart==='Renko')for(const [at,value]of [[layout.modeIndex,layout.sizeIndex],[layout.exitModeIndex,layout.exitSizeIndex]])if(Number.isInteger(at))current[at].addEventListener('change',()=>{const reset=()=>{current[value].value=['ATR','ATR %'].includes(current[at].selectedOptions[0].textContent)?'14':'1';};if(mode==='market-renko-delayed')nativeTimeout(reset,650);else reset();});
  }
 }
 const marketButton=button(main,'Market Trend Filter',()=>{const p=popup('Market Trend Filter');render(p,'marketFilter',filterState);button(p,'Save',()=>{filterState=fields(p);filterSaves++;p.remove();});});marketButton.disabled=true;
 render(main,'momentum',mainState);const done=d.createElement('span');done.textContent='BackTest Completed.';main.append(done);let backtest;
 backtest=button(main,'BackTest',()=>{const p=popup('Momentum Trading BackTest');render(p,'execution',sourceTemplate.stages.execution.fields);button(p,'Backtest',()=>{submissions++;backtest.textContent='Cancel BackTest';done.textContent='Processing';nativeTimeout(()=>{backtest.textContent='BackTest';done.textContent='BackTest Completed.';p.remove();},160);});});
 button(main,'Portfolio Testing',()=>{const p=popup('Portfolio Backtesting');render(p,'portfolio',sourceTemplate.stages.portfolio.fields);button(p,'Backtest',()=>{portfolios++;const report=popup('Portfolio Backtesting Report'),tabs=d.createElement('div');for(const name of ['Quick Stats','Statistics','Charts','Trade Details']){const tab=d.createElement('div');tab.setAttribute('role','tab');tab.textContent=name;tab.onclick=()=>{for(const t of tabs.children)t.className='';tab.className='selected';};tabs.append(tab);}report.append(tabs);const panel=d.createElement('div');panel.setAttribute('role','tabpanel');panel.innerHTML='<div class="stats-card"><div class="status">Total no. of Trades</div><div class="amt">1</div></div><div class="stats-card"><div class="status">Gross Total Returns( % )</div><div class="amt">10%</div></div><table class="dropdown-table-body"><tr><td>Group</td><td>Demo universe 40</td></tr></table>'+Array.from({length:6},(_,i)=>'<svg class="highcharts-root" xmlns="http://www.w3.org/2000/svg"><title>Chart '+i+'</title><path d="M0 0 L20 10"/></svg>').join('')+'<table class="rade-result-detail"><tr><th>Sr #</th><th>Symbol</th><th>Qty</th></tr><tr><td>1</td><td>FICTIONAL</td><td>10</td></tr></table><span id="curPageTextEle">1</span><span id="lastPageTextEle">1</span><img src="/firstPage.png"><img src="/next.png">';report.append(panel);});});
 const runtime={id:'filter-test',getURL:path=>'chrome-extension://filter-test/'+path},source={id:runtime.id,url:w.location.href,tab:{id:9}},dashboard={id:runtime.id,url:runtime.getURL('index.html')},storage={get:async key=>E.clone(key?{[key]:memory[key]??null}:memory),set:async value=>Object.assign(memory,E.clone(value))};let uuid=0;
 const coordinator=createCoordinator({storage,runtime,uuid:()=> 'filter-id-'+(++uuid)});w.chrome={storage:{local:storage},runtime:{...runtime,sendMessage:message=>coordinator.handle(message,source),onMessage:{addListener:fn=>listeners.push(fn)}}};w.URL.createObjectURL=()=> 'blob:filter';w.URL.revokeObjectURL=()=>{};
 for(const name of ['core.js','presentation.js','intelligence.js','source-layouts.js','setup.js','experiments.js','capture.js','runner.js'])w.eval(fs.readFileSync(path.join(base,name),'utf8'));
 const request=data=>new Promise(resolve=>{for(const fn of listeners)fn(data,{id:runtime.id},resolve);});
 const statusProbe=d.createElement('span');w.VaultCapture.addControl(statusProbe);const hostStatus=()=>statusProbe.nextElementSibling.textContent;
 try{
  if(dailySeed){nodes(main)[12].value='444';const at=L.marketFilter(filterState).methodValueIndices[0],fullAt=L.marketFilter(filterFull).methodValueIndices[0];filterState[at].value='27';filterFull[fullAt].value='27';}
  for(let n=0;n<30&&!memory['runner:tab:9'];n++)await sleep(10);const initial=JSON.stringify(fields(main)),loaded=await request({type:'vault-runner-config',...(dailySeed?{cachedChoices:dailySeed.records}:{})});assert.equal(loaded.ok,true,loaded.error);assert.equal(JSON.stringify(fields(main)),initial);assert.equal(submissions,0);let template=loaded.config;const readerResponses=[loaded];
  if(mode==='rs-warm'){const warm=await request({type:'vault-runner-config',warmChart:'Candle',cachedChoices:[loaded.choiceCache]});assert.equal(warm.ok,true,warm.error);readerResponses.push(warm);assert.equal(warm.config.stages.momentum.fields.length,52,'Primary daily coverage uses the RS-off Candle shape.');assert.equal(warm.choiceCache.stages.momentum.context.relativeStrength,false);assert.equal(warm.config.stages.execution.fields[4].value,'Price');assert.equal(JSON.stringify(fields(main)),initial,'Warming the same chart restores the originally enabled Relative Strength fields exactly.');assert.equal(selectionIDs(),originalSelectionIDs);assert.equal(filterSaves,0);assert.equal(submissions,0);}
  for(const loadFilter of variantRS?['relativeStrength']:['relative-strength','symbol-identity','rs-warm','symbol-no-match','symbol-catalogue'].includes(mode)?['relativeStrength']:mode.startsWith('market-')?['marketFilter']:['relativeStrength','marketFilter']){const priorClears=symbolQueryClears,response=await request({type:'vault-runner-config',loadFilter,...(dailySeed?{cachedChoices:dailySeed.records}:{})});if(['market-unknown-dialog','market-symbol-timeout'].includes(mode)){assert.equal(response.ok,false);if(mode==='market-unknown-dialog'){assert.match(response.error,/open menu above its settings/i);assert.ok(w.VaultCapture.popup('Unexpected source notice'),'Unknown dialogs remain visible for the user rather than being treated as a symbol shell.');assert.ok(w.VaultCapture.popup('Market Trend Filter'),'Keep the parent intact when an unrelated child blocks its close.');}else{assert.match(response.error,/did not return confirmed symbols/i);assert.ok(symbolQueryClears>priorClears,'A spinner that never finishes is still dismissed through its native empty query.');noSymbolMenus();}assert.equal(submissions,0);assert.equal(filterSaves,0);assert.equal(commits.length,0);assert.equal(selectionIDs(),originalSelectionIDs);if(mode!=='market-unknown-dialog')assert.equal(JSON.stringify(fields(main)),initial);return;}assert.equal(response.ok,true,response.error);readerResponses.push(response);template=response.config;assert.equal(JSON.stringify(fields(main)),initial,'Optional discovery restores exact main controls.');assert.equal(w.VaultCapture.popup('Market Trend Filter'),undefined);assert.equal(filterSaves,0,'Discovery closes its owned dialog without committing changed settings.');if(!dailySeed)assert.ok(symbolQueryClears>priorClears,'Native empty-query dismissal is exercised during filter discovery.');assert.equal(selectionIDs(),originalSelectionIDs,'Discovery preserves every original native symbol identity.');noSymbolMenus();}
  if(mode==='symbol-catalogue'){
   const rs=L.main('Candle',true),assertPool=options=>{const native=options.filter(option=>option.label.startsWith('N catalogue '));assert.equal(native.length,200,'All 200 observed native symbols survive cross-role reuse.');assert.ok(native.every(option=>option.market==='NSE'&&!option.disabled&&option.sourceValue===option.label+'|catalogue:'+Number(option.label.slice(-3))),'A BSE duplicate name cannot disable an NSE source identity.');};let records=dailySeed?.records;
   if(!dailySeed){
    const found=await request({type:'vault-runner-symbol-search',stage:'momentum',fieldIndex:rs.benchmarkIndex,market:'NSE',query:'N',session:template.session});assert.equal(found.ok,true,found.error);assert.equal(found.result.options.length,200,'Native search is not restricted to a 50-row preview.');assertPool(found.result.options);
    missingSymbols=true;const empty=await request({type:'vault-runner-symbol-search',stage:'momentum',fieldIndex:rs.benchmarkIndex,market:'NSE',query:'Absent fictional benchmark',session:template.session});missingSymbols=false;assert.equal(empty.ok,true,empty.error);assert.equal(empty.result.options.length,0,'The exact empty query remains empty.');assertPool(empty.result.choiceCache.stages.momentum.symbolOptions[rs.benchmarkIndex]);
    const before=scans.symbols,retained=await request({type:'vault-runner-config',loadFilter:'relativeStrength'});assert.equal(retained.ok,true,retained.error);assert.equal(scans.symbols,before,'The current source seed is already known; no repeated symbol walk is needed.');assertPool(retained.config.stages.momentum.relativeStrength.options[rs.benchmarkIndex]);records=await persistedMenus([loaded,retained],{reorderStorage:true});
   }else{assert.notEqual(loaded.session,dailySeed.session);assertPool(template.stages.momentum.relativeStrength.options[rs.benchmarkIndex]);assert.equal(scans.symbols,0,'A fresh source document reuses the observed daily symbols.');}
   const cachedSymbols=[{value:'N catalogue 001',label:'N catalogue 001',sourceValue:'N catalogue 001|BSE:1',market:'BSE',disabled:false},{value:'Receipt only',label:'Receipt only',sourceValue:'Receipt only|NSE:receipt',market:'NSE',disabled:false}],read=await request({type:'vault-runner-config',loadFilter:'marketFilter',cachedChoices:dailySeed?records:[],cachedSymbols});assert.equal(read.ok,true,read.error);const mtf=L.marketFilter(read.config.stages.marketFilter.fields);
   for(const role of ['indexSymbolIndex','numeratorSymbolIndex','denominatorSymbolIndex']){const options=read.config.stages.marketFilter.options[mtf[role]];assertPool(options);assert.ok(options.some(option=>option.label==='Receipt only'&&option.sourceValue==='Receipt only|NSE:receipt'&&!option.disabled),'The coordinator query receipt is shared with each same-market filter role.');assert.ok(options.every(option=>option.market==='NSE'));}
   assert.equal(JSON.stringify(fields(main)),initial);assert.equal(selectionIDs(),originalSelectionIDs,'Search, empty results, cross-role reads and restored dialog preserve original native selections.');assert.equal(w.VaultCapture.popup('Market Trend Filter'),undefined);noSymbolMenus();assert.equal(submissions,0);assert.equal(portfolios,0);assert.equal(filterSaves,0);assert.equal(orderClicks,0);assert.equal(commits.length,0);
   if(!dailySeed)return await filterScenario(mode,{records,session:loaded.session});return;
  }
  if(mode==='symbol-no-match'){
   missingSymbols=true;const layout=L.main('Candle',true),before=JSON.stringify(fields(main)),priorIDs=selectionIDs(),read=await request({type:'vault-runner-symbol-search',stage:'momentum',fieldIndex:layout.benchmarkIndex,market:'NSE',query:'Absent fictional benchmark',session:template.session});assert.equal(read.ok,true,read.error);assert.equal(read.result.options.length,0,'A stable native no-match result is a confirmed empty lookup.');assert.equal(JSON.stringify(fields(main)),before);assert.equal(selectionIDs(),priorIDs);noSymbolMenus();
   const normalized=S.template(template),experiment=E.create({id:'no-symbol-proof',name:'Missing source benchmark',baseline:S.configToBaseline({...S.defaults(normalized),'momentum.rs':true},normalized),dimensions:[],objective:'returns',minTrades:0}),priorCommits=commits.length;await assert.rejects(w.VaultRunner.apply(main,experiment,experiment.trials[0],'momentum'),/benchmark.*unavailable|ambiguous/i);assert.ok(noMatchLists>=2,'Read and choose both exercise the native empty-symbol shell.');assert.equal(nodes(main)[layout.benchmarkIndex].value,'Demo benchmark','Failed selection restores the original displayed source symbol.');assert.equal(selectionIDs(),priorIDs);assert.equal(commits.length,priorCommits);noSymbolMenus();assert.equal(submissions,0);assert.equal(portfolios,0);assert.equal(filterSaves,0);assert.equal(orderClicks,0);return;
  }
  if(mode==='rs-warm'){
   const records=await persistedMenus(readerResponses),before={...scans},connected=await request({type:'vault-runner-config',cachedChoices:records});assert.equal(connected.ok,true,connected.error);assert.equal(connected.choicesFromCache,true);assert.deepEqual(scans,before,'Normal RS-on reconnect reuses cached menus and benchmark metadata.');const layout=L.main('Candle',true),benchmark=connected.config.stages.momentum.options[layout.benchmarkIndex];assert.ok(benchmark.some(option=>option.label==='Demo benchmark'&&option.sourceValue==='Demo benchmark|momentum:benchmarkIndex'&&option.market==='NSE'));assert.equal(connected.config.stages.momentum.symbolQueries[layout.benchmarkIndex].query,'Demo benchmark');assert.equal(JSON.stringify(fields(main)),initial);assert.doesNotThrow(()=>S.template(connected.config));
   const NativeDate=w.Date,offset=36*60*60*1000;w.Date=class extends NativeDate{constructor(...args){super(...(args.length?args:[NativeDate.now()+offset]));}static now(){return NativeDate.now()+offset;}};const oldCounts={...scans},expired=await request({type:'vault-runner-config',loadFilter:'relativeStrength',cachedChoices:records});assert.equal(expired.ok,false);assert.match(expired.error,/new day has started/i);assert.deepEqual(scans,oldCounts,'Overnight optional reuse must reconnect before opening or walking stale controls.');const found=await request({type:'vault-runner-symbol-search',stage:'momentum',fieldIndex:layout.benchmarkIndex,market:'NSE',query:'Demo benchmark',session:connected.session});assert.equal(found.ok,true,found.error);assert.ok(found.result.options.length);assert.equal(Object.hasOwn(found.result,'choiceCache'),false,'A fresh query cannot renew yesterday’s broader menus.');assert.equal(JSON.stringify(fields(main)),initial);assert.equal(selectionIDs(),originalSelectionIDs);assert.equal(submissions,0);assert.equal(filterSaves,0);return;
  }
  if(mode==='daily-cache'){
   if(!dailySeed){assert.ok(scans.groups>0&&scans.categories>0&&scans.symbols>=4);const records=await persistedMenus(readerResponses);assert.ok(records.some(record=>record.stages.marketFilter?.symbolQueries),'Optional symbol metadata crosses the actual durable boundary.');return await filterScenario(mode,{records,session:loaded.session});}
   assert.notEqual(loaded.session,dailySeed.session);assert.deepEqual(scans,{groups:0,categories:0,symbols:0},'A new document reuses native/private/Group and benchmark menus without rediscovery.');assert.ok(readerResponses.every(response=>response.choicesFromCache));assert.equal(template.stages.momentum.fields[12].value,'444');const layout=L.marketFilter(template.stages.marketFilter.current.fields);assert.equal(template.stages.marketFilter.current.fields[layout.methodValueIndices[0]].value,'27','Current filter inputs must come from the source, never stored menus.');assert.equal(selectionIDs(),originalSelectionIDs);assert.equal(filterSaves,0);assert.equal(submissions,0);
   const before={...scans},forced=await request({type:'vault-runner-config',loadFilter:'marketFilter',cachedChoices:dailySeed.records,forceChoices:true});assert.equal(forced.ok,true,forced.error);assert.equal(forced.choicesFromCache,false);assert.ok(scans.categories>before.categories&&scans.symbols>before.symbols,'Manual refresh bypasses durable and in-memory optional caches.');assert.equal(scans.groups,before.groups,'A filter refresh does not walk unrelated Group choices.');
   const normalized=S.template(template),config={...S.defaults(normalized),'momentum.rs':true},experiment=E.create({id:'daily-symbol-proof',name:'Cached choices require fresh selection',baseline:S.configToBaseline(config,normalized),dimensions:[],objective:'returns',minTrades:0}),priorCommits=commits.length,priorQueries=scans.symbols;changeSymbolIdentity=true;await assert.rejects(w.VaultRunner.apply(main,experiment,experiment.trials[0],'momentum'),/benchmark.*unavailable|ambiguous/i);assert.ok(scans.symbols>priorQueries,'Trial application must still query the real source before committing a cached benchmark.');assert.equal(commits.length,priorCommits);assert.equal(submissions,0);assert.equal(orderClicks,0);return;
  }
  if(mode==='market-late-symbol'){await sleep(350);assert.ok(lateSymbolReopens>=3,'Each benchmark lookup sees its delayed native response reopen after dismissal.');assert.ok(symbolQueryClears>=6,'Late result lists are dismissed through their own native empty query.');noSymbolMenus();assert.equal(JSON.stringify(fields(main)),initial);assert.equal(selectionIDs(),originalSelectionIDs);assert.equal(w.VaultCapture.popup('Market Trend Filter'),undefined);assert.equal(filterSaves,0);assert.equal(submissions,0);assert.equal(portfolios,0);return;}
  if(mode==='market-filter'){const history=[],observer=new w.MutationObserver(()=>history.push(hostStatus()));observer.observe(statusProbe.parentNode,{childList:true,subtree:true,characterData:true});const rejected=await request({type:'vault-runner-config',changes:{marketFilter:{999:'Invalid selector'}}});await sleep(0);observer.disconnect();assert.equal(rejected.ok,false);assert.match(rejected.error,/market trend selector cannot refresh/i);assert.ok(history.some(value=>/^Reading Market Trend Filter settings/.test(value)),'The rejected operation actually entered source filter discovery.');assert.equal(hostStatus(),'Could not load settings: '+rejected.error,'The source host must show the terminal error, not leave a completed request looking active.');assert.doesNotMatch(hostStatus(),/^Reading /);assert.equal(JSON.stringify(fields(main)),initial);assert.equal(w.VaultCapture.popup('Market Trend Filter'),undefined);assert.equal(filterSaves,0);assert.equal(submissions,0);}
  if(mode==='symbol-identity'){const layout=L.main('Candle',true),response=await request({type:'vault-runner-config',changes:{momentum:{[layout.benchmarkMarketIndex]:'All'}}});assert.equal(response.ok,true,response.error);template=response.config;const search=await request({type:'vault-runner-symbol-search',stage:'momentum',fieldIndex:layout.benchmarkIndex,market:'All',query:'Demo benchmark',session:template.session});assert.equal(search.ok,true,search.error);template.stages.momentum.relativeStrength.options[layout.benchmarkIndex]=search.result.options;}
  if(['market-renko','market-renko-delayed'].includes(mode)){for(const key of ['modeIndex','exitModeIndex']){const layout=L.marketFilter(template.stages.marketFilter.fields),response=await request({type:'vault-runner-config',changes:{marketFilter:{[layout[key]]:'ATR'}}});assert.equal(response.ok,true,response.error);template=response.config;const updated=L.marketFilter(template.stages.marketFilter.fields);assert.equal(template.stages.marketFilter.fields[updated.modeIndex].value,'ATR','The second context refresh must retain the first chosen ATR mode.');if(key==='exitModeIndex')assert.equal(template.stages.marketFilter.fields[updated.exitModeIndex].value,'ATR');assert.equal(JSON.stringify(fields(main)),initial);assert.equal(filterSaves,0);}}
  if(mode==='market-context'){
   const layout=L.marketFilter(template.stages.marketFilter.fields),before=JSON.stringify(filterState);assert.equal(L.marketFilter(filterState).hasExit,false);
   const response=await request({type:'vault-runner-config',changes:{marketFilter:{[layout.rows[0].parentIndex]:'Popular'}}});assert.equal(response.ok,true,response.error);const changed=response.config.stages.marketFilter,expanded=L.marketFilter(changed.fields);assert.equal(changed.fields[expanded.rows[0].parentIndex].value,'Popular');assert.ok(changed.options[expanded.rows[0].childIndex].some(option=>option.label===expanded.rows[0].name+' popular'));assert.equal(JSON.stringify(filterState),before,'Reading an exit category from an Exit-only source must not save changed source settings.');assert.equal(JSON.stringify(fields(main)),initial);assert.equal(filterSaves,0);assert.equal(submissions,0);assert.equal(w.VaultCapture.popup('Market Trend Filter'),undefined);return;
  }
  if(mode==='market-retained'){
   const layout=L.marketFilter(template.stages.marketFilter.fields);for(const key of ['indexSymbolIndex','numeratorSymbolIndex','denominatorSymbolIndex']){const index=layout[key],query='Changed '+template.stages.marketFilter.fields[index].value,priorIDs=selectionIDs(),priorClears=symbolQueryClears,response=await request({type:'vault-runner-symbol-search',stage:'marketFilter',fieldIndex:index,market:'NSE',query,session:template.session});assert.equal(response.ok,true,response.error);template.stages.marketFilter.options[index].push(...response.result.options);assert.equal(JSON.stringify(fields(main)),initial);assert.equal(selectionIDs(),priorIDs,'Reading pending benchmark alternatives preserves original native identities.');assert.ok(symbolQueryClears>priorClears);noSymbolMenus();}
  }
  const normalized=S.template(template),config=S.defaults(normalized);assert.equal(submissions,0);assert.equal(portfolios,0);assert.equal(orderClicks,0);
  if(variantRS){
   let selectedTemplate=normalized;const saved=[];
   for(const [index,[on,category]]of [[true,'Pre'],[false,'Pre'],[true,'Popular']].entries()){
    if(category==='Popular'){const refreshed=await request({type:'vault-runner-config',changes:{momentum:{60:'Popular'}}});assert.equal(refreshed.ok,true,refreshed.error);selectedTemplate=S.template(refreshed.config);}
    const settings={...S.defaults(selectedTemplate),'momentum.rs':on,'momentum.rs.source':category,'momentum.rs.rule':category==='Popular'?'Relative Strength popular':'Demo relative rule'},study=E.create({id:'variant-rs-'+index,name:rsChart+' RS transition '+index,baseline:S.configToBaseline(settings,selectedTemplate,{id:'variant-rs-base-'+index,name:'RS controls',demo:false}),dimensions:[],objective:'returns',minTrades:0});memory['experiment:'+study.id]=study;
    await request({type:'vault-runner-wake'});await sleep(100);await coordinator.handle({action:'start',id:study.id,tabId:9},dashboard);await request({type:'vault-runner-wake'});
    for(let n=0;n<1200&&!['complete','needs-review'].includes(memory['experiment:'+study.id].status);n++)await sleep(100);
    const result=memory['experiment:'+study.id];assert.equal(result.status,'complete',JSON.stringify(result.trials.map(t=>({status:t.status,error:t.error}))));const trial=result.trials[0],run=memory['run:'+trial.runId];assert.ok(run);E.verifySettings(E.expected(study,trial),run);const actual=E.fields(run,'momentum'),shape=L.stage('momentum',actual);assert.equal(shape.relativeStrength,on);assert.equal(actual.length,on?62:58);for(const at of [shape.sizeIndex,shape.modeIndex])assert.ok(actual[at].label.startsWith('RS SB : / '+(on?category:'Public')+'i → '),'Preserve the actual source construction heading while comparing its semantic setting.');assert.equal(run.provenance,'recorded-at-submit');assert.equal(run.trades.rows.length,1);assert.equal(run.charts.length,6);assert.ok(run.experiment.evidence.strategySubmissionId);assert.ok(run.experiment.evidence.portfolioSubmissionId);saved.push(run);
   }
   assert.equal(submissions,3);assert.equal(portfolios,3);assert.equal(filterSaves,0);assert.equal(orderClicks,0);assert.equal(new Set(saved.map(r=>r.experiment.evidence.strategySubmissionId)).size,3);assert.equal(new Set(saved.map(r=>r.experiment.evidence.portfolioSubmissionId)).size,3);assert.equal(memory['runner:lease'],null);return;
  }
  if(mode==='market-delayed-symbol'){assert.ok(pendingSymbolShells>=3,'Every benchmark lookup exercised native transient loading.');assert.ok(staleSymbolLists>0,'A cached list was replaced by a spinner before fresh results.');assert.ok(Object.values(template.stages.marketFilter.options).flat().every(option=>!option.sourceValue?.includes('|stale:')),'Only the completed fresh list can supply symbol identities.');}
  if(mode==='relative-strength'){
   const layout=L.main('Candle',true),snapshot=JSON.stringify(fields(main)),priorIDs=selectionIDs(),priorClears=symbolQueryClears,result=await request({type:'vault-runner-symbol-search',stage:'momentum',fieldIndex:layout.benchmarkIndex,market:'All',query:'Demo benchmark',session:template.session});assert.equal(result.ok,true,result.error);assert.equal(result.result.options[0].market,'NSE');assert.equal(JSON.stringify(fields(main)),snapshot,'Symbol lookup restores off-state and retained settings.');assert.equal(selectionIDs(),priorIDs,'Searching another market restores the original committed symbol identity.');assert.ok(symbolQueryClears>priorClears);noSymbolMenus();
   const experiment=E.create({id:'filter-rs',name:'Relative Strength apply',baseline:S.configToBaseline(config,normalized,{id:'filter-base',name:'Filters'}),dimensions:[{key:'momentum.rs',values:[true,false]}],objective:'returns',minTrades:0});for(const trial of experiment.trials){await w.VaultRunner.apply(main,experiment,trial,'momentum');E.verify(E.fields(E.expected(experiment,trial),'momentum'),fields(main));}assert.ok(commits.some(commit=>commit.stage==='momentum'&&commit.index===layout.benchmarkIndex));assert.equal(fields(main).length,52);assert.equal(orderClicks,0);return;
  }
  if(mode==='symbol-identity'){
   const experiment=E.create({id:'filter-proof',name:'Changed benchmark proof',baseline:S.configToBaseline({...config,'momentum.rs':true,'momentum.rs.market':'All','momentum.rs.benchmark':'Demo benchmark'},normalized),dimensions:[],objective:'returns',minTrades:0}),priorCommits=commits.length;changeSymbolIdentity=true;await assert.rejects(w.VaultRunner.apply(main,experiment,experiment.trials[0],'momentum'),/benchmark.*unavailable|ambiguous/i);assert.equal(commits.length,priorCommits);changeSymbolIdentity=false;changeSymbolExchange=true;await assert.rejects(w.VaultRunner.apply(main,experiment,experiment.trials[0],'momentum'),/benchmark.*unavailable|ambiguous/i,'All-market lookup must still commit the saved exchange, even if source identity and caption are unchanged.');assert.equal(commits.length,priorCommits);assert.equal(submissions,0);assert.equal(orderClicks,0);return;
  }
  const finalConfig={...config,'momentum.market-filter':true,...(mode==='combined'?{'momentum.rs':true}:{}),...(mode==='market-retained'?{'marketFilter.method':'D Smart','marketFilter.ema':37,'marketFilter.dsmart':41,'marketFilter.mast':53,'marketFilter.ktqp':67,'marketFilter.index.symbol':'Changed Demo index','marketFilter.numerator.symbol':'Changed Demo numerator','marketFilter.denominator.symbol':'Changed Demo denominator'}:{}),...(['market-renko','market-renko-delayed'].includes(mode)?{'marketFilter.brick.size':17,'marketFilter.exit.brick.size':21}:{})},plan=E.create({id:'filter-proof',name:'Verified filter submissions',baseline:S.configToBaseline(finalConfig,normalized,{id:'filter-baseline',name:'Filters',demo:false}),dimensions:[{key:'marketFilter.mode',values:['Index','RS']}],objective:'returns',minTrades:0});memory['experiment:'+plan.id]=plan;
  if(['market-renko','market-renko-delayed'].includes(mode)){filterState=L.projectMarketFilter(sourceTemplate.stages.marketFilter.fields,{mode:'Index',action:L.marketActions[2]});const layout=L.marketFilter(filterState);for(const index of [layout.modeIndex,layout.exitModeIndex])filterState[index].value='Percent';for(const index of [layout.sizeIndex,layout.exitSizeIndex])filterState[index].value='1';}
  for(let n=0;n<30&&!memory['runner:tab:9']?.ready;n++)await sleep(10);if(mode==='market-renko-delayed'){await request({type:'vault-runner-wake'});await sleep(100);}await coordinator.handle({action:'start',id:plan.id,tabId:9},dashboard);await request({type:'vault-runner-wake'});for(let n=0;n<1600&&!['complete','needs-review'].includes(memory['experiment:'+plan.id].status);n++)await sleep(20);const result=memory['experiment:'+plan.id];assert.equal(result.status,'complete',JSON.stringify(result.trials.map(t=>({status:t.status,error:t.error}))));assert.equal(submissions,2);assert.equal(portfolios,2);assert.equal(filterSaves,2);assert.equal(orderClicks,0);
  for(const trial of plan.trials){const run=memory['run:'+trial.runId];assert.ok(run);E.verifySettings(E.expected(plan,trial),run);assert.equal(run.parameters.strategy.auxiliarySettingsUncaptured,false);assert.ok(run.parameters.strategy.marketTrend.at);const layout=L.marketFilter(E.fields(run,'marketFilter'));assert.equal(E.fields(run,'marketFilter')[layout.rsModeIndex].checked,trial.patch['marketFilter.mode']==='RS');assert.ok(run.experiment.evidence.strategySubmissionId);}
  if(mode==='market-filter'){assert.ok(w.VaultCapture.getMarketTrend(),'The successful source Save has a current receipt.');const error=popup('Error');assert.equal(w.VaultCapture.getMarketTrend(),null,'A source error immediately makes the receipt unusable.');error.querySelector('.close-buton').click();assert.equal(w.VaultCapture.getMarketTrend(),null,'Dismissing a source error must not resurrect its earlier Save receipt.');for(const trial of plan.trials)E.verifySettings(E.expected(plan,trial),memory['run:'+trial.runId]);}
 }finally{try{noSymbolMenus();}finally{dom.window.close();}}
}
(async()=>{
 ...
 const cases=[
  ...
  {chartCase:'daily-categories'},{filterCase:'daily-cache'},{vaultSetup:true,closeSlowAnimation:true},{vaultSetup:true,closeBlockedByChild:true},{filterCase:'market-late-symbol'},{filterCase:'rs-warm'},{filterCase:'symbol-no-match'},{chartCase:'daily-storage-order'},{filterCase:'symbol-catalogue'},{chartCase:'exit-context'},{chartCase:'dependent-timeout'},{chartCase:'daily-passive-state'},{chartCase:'saved-visited-layout'},
  ...['Candle','P&F','Renko'].flatMap(mainChart=>['Candle','P&F','Renko'].filter(exitChart=>mainChart!=='Candle'||exitChart!=='Candle').map(exitChart=>({variantLifecycle:{mainChart,exitChart}}))),
  ...['Absolute','ATR','ATR %'].map(mode=>({variantLifecycle:{mainChart:'Renko',exitChart:'Renko',mainMode:mode,exitMode:mode}})),
  {variantLifecycle:{mainChart:'P&F',exitChart:'Renko',failure:'stale'}},{variantLifecycle:{mainChart:'Renko',exitChart:'P&F',failure:'duplicate'}},{filterCase:'market-renko-delayed'},{filterCase:'relative-strength-pnf'},{filterCase:'relative-strength-renko'}
 ];
 ...
 for(const [index,options]of selected.slice(fromCase-1,lastCase).entries()){
  const run=()=>options.variantLifecycle?variantLifecycle(options.variantLifecycle):options.filterCase?filterScenario(options.filterCase):options.chartCase?chartScenario(options.chartCase):scenario(options);
  if(options.variantLifecycle||['P&F','Renko','dependent-timeout','daily-passive-state','saved-visited-layout'].includes(options.chartCase)||['rs-warm','market-renko-delayed','relative-strength-pnf','relative-strength-renko'].includes(options.filterCase))await withFixtureTime(run);else await run();console.log('PASS: runner '+(index+fromCase)+'/'+selected.length+' '+(Object.keys(options).length?JSON.stringify(options):'baseline sequence'));
 }
