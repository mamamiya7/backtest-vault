...
(async()=>{const demos=app({},true);let real,restored;try{
 ...
 await real.d.getElementById('backup').onclick();const backup=JSON.parse(await real.downloads.at(-1).text());assert.equal(backup.benchmarks.length,1);assert.equal(backup.runs.length,1);assert.match(real.d.querySelector('#detail').textContent,/Sparse observations/);
 ...
 await restored.d.getElementById('backup').onclick();const roundtrip=JSON.parse(await restored.downloads.at(-1).text());assert.deepEqual(roundtrip.runs,backup.runs);assert.deepEqual(roundtrip.benchmarks,backup.benchmarks);
