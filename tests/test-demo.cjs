...
(async()=>{try{
 ...
 await d.getElementById('backup').onclick();const archive=JSON.parse(await downloads.at(-1).text());assert.equal(archive.runs.length,6);assert.ok(archive.runs.every(r=>r.demo));assert.ok(names.at(-1).startsWith('demo-'));
