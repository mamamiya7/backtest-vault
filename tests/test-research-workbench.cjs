...
async function check(main,exit,filter){
 ...
 try{
  ...
  const capability=w.VaultSetup.executionCapability(source);assert.equal(capability.available,main==='Candle'&&exit==='Candle','UI does not widen execution eligibility');
 }finally{w.VaultExperimentsUI.dispose();dom.window.close();}
}
(async()=>{let layouts=0;for(const main of ['Candle','P&F','Renko'])for(const exit of ['Candle','P&F','Renko'])for(const filter of ['Candle','P&F','Renko']){await check(main,exit,filter);layouts++;}console.log('PASS: '+layouts+' independent chart layouts, all model fields rendered once; 864 Market Trend mode/method/action states; native gates, Radar/RS, filter-window reopen/Escape, cancel, no extra discovery, unchanged execution eligibility.');})().catch(error=>{console.error(error);process.exitCode=1;});
