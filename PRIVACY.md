...
New test reads dropdown labels and available Group names from the signed-in RZone page, including offered categories for Radar, Strategy 1–3 and Exit Strategy. A daily menu cache is stored in Chrome extension storage, scoped to the RZone tab, document session, local calendar day and form adapter. It contains offered Group/rule names and menu context, not saved results or a reusable settings snapshot. It is separate from research backups and is ignored after a source reload, a new day or adapter changes. Recheck all choices invalidates that connection's cached menus before scanning again.

STR/exit My/Public searches send the keyword you enter through the selected RZone tab's normal search UI and collect its matching results. Search results are not added to the daily native-menu cache. A created experiment stores its source-choice snapshot and search queries locally, so experiment exports/backups may contain account-specific Group and rule names, including choices not selected for that experiment. Filtering loaded choices and switching cached categories happen locally. Opening menus, searching or changing a parent selector can trigger RZone's normal page requests; Vault does not send these choices to another service.
