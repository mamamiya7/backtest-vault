...
New test reads dropdown labels and available Group names from the signed-in RZone page, including offered categories for Radar, Strategy 1–3 and Exit Strategy. A daily menu cache is stored in Chrome extension storage, scoped to this browser profile, local calendar day and form adapter. It contains offered names and menu context, not saved results or a reusable settings snapshot. It is separate from research backups, survives source reloads and expires on a new day or adapter change. Recheck all choices invalidates the daily menus before scanning again.

STR/exit My/Public searches send the keyword you enter through the selected RZone tab's normal search UI and collect its matching results. Daily dropdown metadata, including Group, private categories, search results and loaded benchmark symbols, is stored locally in this browser profile and reused across RZone tabs and refreshes. Use Recheck all choices after changing accounts or rules. Current form values are not stored in that menu cache. A created experiment separately stores its source-choice snapshot and search queries locally, so experiment exports/backups may contain account-specific Group and rule names, including choices not selected for that experiment. Filtering loaded choices and switching cached categories happen locally. Opening menus, searching or changing a parent selector can trigger RZone's normal page requests; Vault does not send these choices to another service.
