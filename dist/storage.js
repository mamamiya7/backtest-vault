window.VaultStore = (() => {
  ...
  if (demo) {
    let memory = window.VaultDemo.create(), benchmarks = window.VaultDemo.benchmarks?.() || [];
    const clone = value => JSON.parse(JSON.stringify(value));
    return {demo:true,all:async()=>clone(memory),put:async run=>{const i=memory.findIndex(r=>r.id===run.id);if(i<0)memory.push(clone(run));else memory[i]=clone(run);},allBenchmarks:async()=>clone(benchmarks),putBenchmark:async()=>{throw Error('Benchmark import is disabled in demo mode.');},reset:async()=>{memory=window.VaultDemo.create();benchmarks=window.VaultDemo.benchmarks?.()||[];}};
  }
  ...
  return {all,put,allBenchmarks,putBenchmark};
})();
