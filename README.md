...
**v0.8.7 preview** · Chrome · Local storage · MIT · No cloud account
...
![One setup page for strategy, periods, dates, exits and portfolio sizing, with two planned tests; fictional sample setup](docs/images/11-test-setup.png)
...
2. **Set up on one page:** periods, weights, filters and Strategy 1–3 follow RZone's arrangement. Dates, rank criteria, execution chart, selection type, exit strategy, target, stop loss and portfolio sizing sit directly below. Search and select **Group** from the choices read from RZone. Arrow keys and Enter also work.
...
**New test** and **Refresh choices** load Group, the offered **Radar** categories (currently **Pre / My**), and the available **Pre / My / Public / Popular** rules for **Strategy 1, 2 and 3**. Vault temporarily enables each source row, reads its categories, then restores the original settings. In Vault, choose **On** beside Radar or a strategy and select its category and rule. Switching loaded categories is immediate; each category keeps its own rule selections and Test values while you edit. Empty categories stay empty. Refresh again after adding rules in RZone; removed choices stay visible for review. Market changes refresh Group; exit-source menus refresh on demand. Loading choices never submits a backtest or needs a separate download.
...
Dates, universe, market, chart model and portfolio assumptions stay shared within a batch. The highlighted setting is the best of the tested candidates under the chosen rules, not a guarantee of future returns.

```mermaid
flowchart LR
    A["Signed-in RZone<br/>Group + offered Radar / STR categories"] -->|"Open New test / Refresh choices"| B["Choose in Vault<br/>Radar · STR1 · STR2 · STR3<br/>Each category keeps its own choices"]
...
| Available in 0.8.7 preview | Acceptance boundary |
| --- | --- |
...
No LLM setup is required. Current automatic setup uses **Candle**, **Price**, **Relative Strength off** and **Market Trend Filter off**. P&F/Renko execution and dynamic RS/filter adapters remain on the delivery plan. Existing captures from those chart families can still be inspected and compared.
...
This copies an explicit public-file allowlist into `releases/backtest-vault-0.8.7-public/` and writes a hash manifest. Private archives, handoffs, local hosting metadata and dependencies are excluded. An existing package is left intact.
